//! Port of `ghidra.program.model.data.Pointer16DataType`.
//!
...
//! this crate, `Pointer16DataType` contributes **no method overrides at all** -- its entire body
//! is two constructors (`Pointer16DataType()`/`Pointer16DataType(DataType dt)`, both forwarding to
//! `super(dt, 2)`, fixing the pointer length at 2 bytes) plus a `static { ClassTranslator.put(...) }`
//! registration of the legacy class name `ghidra.program.model.data.Pointer16`.
//!
//! Neither is representable on a trait: traits cannot declare constructors or store fields (so the
//! fixed-length-2 behavior has nowhere to live except
//! [`PointerDataType::stored_length`](crate::program::model::data::pointer_data_type::PointerDataType::stored_length),
//! which a concrete implementation must simply initialize to [`POINTER16_LENGTH`]), and
...
//! `stored_length()` fixed at [`POINTER16_LENGTH`].
//!
...
/// Fixed pointer length (in bytes) for a [`Pointer16DataType`], standing in for the literal `2`
/// passed to the `PointerDataType(DataType, int)` superclass constructor by both
/// `Pointer16DataType` constructors.
pub const POINTER16_LENGTH: i32 = 2;

/// Pointer16 is really a factory for generating 2-byte pointers.
///
/// Port of `ghidra.program.model.data.Pointer16DataType`. See the module-level documentation for
/// why this trait adds no new methods beyond what
/// [`PointerDataType`] already provides.
pub trait Pointer16DataType: PointerDataType {}
...
mod tests {
    ...
    impl DataOrganization for MockDataOrganization {
        ...
        fn get_pointer_size(&self) -> i32 {
            // Deliberately different from POINTER16_LENGTH, so tests can confirm the *stored*
            // fixed length (2) wins over whatever the data organization's native pointer size is.
            8
        }
    ...
    struct MockPointer16DataType {
        referenced: Option<Box<dyn DataType>>,
    }

    impl DataType for MockPointer16DataType {
    ...
    impl BuiltInDataType for MockPointer16DataType {
    ...
    impl Pointer for MockPointer16DataType {
    ...
    impl PointerDataType for MockPointer16DataType {
        ...
        fn set_stored_length(&mut self, _length: i32) {
            // Pointer16DataType's length is always fixed at POINTER16_LENGTH; a real
    ...
    impl Pointer16DataType for MockPointer16DataType {}
    ...
    fn usable_as_trait_object() {
        let dt = MockPointer16DataType { referenced: None };
        let dyn_dt: &dyn Pointer16DataType = &dt;
        assert_eq!(dyn_dt.stored_length(), POINTER16_LENGTH);
    }

    #[test]
    fn length_is_fixed_at_2_bytes_regardless_of_data_organization_pointer_size() {
        let dt = MockPointer16DataType { referenced: None };
        // The mock DataOrganization reports an 8-byte native pointer size, but a Pointer16DataType
        // always reports 2 -- exactly mirroring `super(dt, 2)` fixing the Java `length` field.
        assert_eq!(DataType::get_length(&dt), POINTER16_LENGTH);
        assert_eq!(dt.get_data_organization().get_pointer_size(), 8);
    }

    #[test]
    fn display_name_reflects_pointer_arithmetic_over_stored_length() {
        let untyped = MockPointer16DataType { referenced: None };
        assert_eq!(untyped.pointer_data_type_impl_display_name(), "pointer16");

        let typed = MockPointer16DataType {
