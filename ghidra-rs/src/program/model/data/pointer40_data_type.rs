//! Port of `ghidra.program.model.data.Pointer40DataType`.
//!
...
//! this crate, `Pointer40DataType` contributes **no method overrides at all** -- its entire body
//! is two constructors (`Pointer40DataType()`/`Pointer40DataType(DataType dt)`, both forwarding to
//! `super(dt, 5)`, fixing the pointer length at 5 bytes) plus a `static { ClassTranslator.put(...) }`
//! registration of the legacy class name `ghidra.program.model.data.Pointer40`.
//!
//! Neither is representable on a trait: traits cannot declare constructors or store fields (so the
//! fixed-length-5 behavior has nowhere to live except
//! [`PointerDataType::stored_length`](crate::program::model::data::pointer_data_type::PointerDataType::stored_length),
//! which a concrete implementation must simply initialize to [`POINTER40_LENGTH`]), and
...
//! `stored_length()` fixed at [`POINTER40_LENGTH`].
//!
...
/// Fixed pointer length (in bytes) for a [`Pointer40DataType`], standing in for the literal `5`
/// passed to the `PointerDataType(DataType, int)` superclass constructor by both
/// `Pointer40DataType` constructors.
pub const POINTER40_LENGTH: i32 = 5;

/// Pointer40 is really a factory for generating 5-byte pointers.
///
/// Port of `ghidra.program.model.data.Pointer40DataType`. See the module-level documentation for
/// why this trait adds no new methods beyond what
/// [`PointerDataType`] already provides.
pub trait Pointer40DataType: PointerDataType {}
...
mod tests {
    ...
    impl DataOrganization for MockDataOrganization {
        ...
        fn get_pointer_size(&self) -> i32 {
            // Deliberately different from POINTER40_LENGTH, so tests can confirm the *stored*
            // fixed length (5) wins over whatever the data organization's native pointer size is.
            8
        }
    ...
    struct MockPointer40DataType {
        referenced: Option<Box<dyn DataType>>,
    }

    impl DataType for MockPointer40DataType {
    ...
    impl BuiltInDataType for MockPointer40DataType {
    ...
    impl Pointer for MockPointer40DataType {
    ...
    impl PointerDataType for MockPointer40DataType {
        ...
        fn set_stored_length(&mut self, _length: i32) {
            // Pointer40DataType's length is always fixed at POINTER40_LENGTH; a real
    ...
    impl Pointer40DataType for MockPointer40DataType {}
    ...
    fn usable_as_trait_object() {
        let dt = MockPointer40DataType { referenced: None };
        let dyn_dt: &dyn Pointer40DataType = &dt;
        assert_eq!(dyn_dt.stored_length(), POINTER40_LENGTH);
    }

    #[test]
    fn length_is_fixed_at_5_bytes_regardless_of_data_organization_pointer_size() {
        let dt = MockPointer40DataType { referenced: None };
        // The mock DataOrganization reports an 8-byte native pointer size, but a Pointer40DataType
        // always reports 5 -- exactly mirroring `super(dt, 5)` fixing the Java `length` field.
        assert_eq!(DataType::get_length(&dt), POINTER40_LENGTH);
        assert_eq!(dt.get_data_organization().get_pointer_size(), 8);
    }

    #[test]
    fn display_name_reflects_pointer_arithmetic_over_stored_length() {
        let untyped = MockPointer40DataType { referenced: None };
        assert_eq!(untyped.pointer_data_type_impl_display_name(), "pointer40");

        let typed = MockPointer40DataType {
