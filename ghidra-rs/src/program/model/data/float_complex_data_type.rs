//! Port of `ghidra.program.model.data.FloatComplexDataType`.
//!
//! Provides a definition of a `complex` built-in data type consisting of two float
//! point numbers in the IEEE 754 double precision format.
//!
//! The size of the float numbers is determined by the program's data organization as
//! defined by the language/compiler spec (unlike the fixed-size `Complex8`/`Complex16`/`Complex32`
//! siblings).
//!
//! Port of `ghidra.program.model.data.FloatComplexDataType`, promoted straight to a trait because it
//! was selected as a dependency-cycle cut-point.
//!
//! The Java class `extends AbstractComplexDataType`, already ported as a trait
//! ([`AbstractComplexDataType`]). Every method `FloatComplexDataType` overrides beyond its
...
//!
//! The constructor (`super("floatcomplex", FloatDataType.dataType, dtm)`) has no trait equivalent
...
//! [`FloatDataType`](crate::program::model::data::float_data_type::FloatDataType)-shaped
...
pub trait FloatComplexDataType: AbstractComplexDataType {
    /// Returns an instance of this DataType using the specified `DataTypeManager` to allow its
    /// use of the corresponding `DataOrganization` while retaining its unique identity.
    ///
    /// Port of `FloatComplexDataType.clone(DataTypeManager)`, which overrides
    ...
    /// [`FloatDataType`](crate::program::model::data::float_data_type::FloatDataType), which computes its own
    /// encoded length from the associated `DataOrganization` at construction time rather than a
    /// fixed constant.
    fn float_complex_clone(&self, dtm: Option<Box<dyn DataTypeManager>>) -> Box<dyn FloatComplexDataType>;
}
...
mod tests {
    ...
    struct MockFloatComplexDataType {
        float_type: MockFloat,
        dtm_tag: Option<&'static str>,
    }

    impl DataType for MockFloatComplexDataType {
        fn get_name(&self) -> String {
            "floatcomplex".to_string()
        }
        fn get_length(&self) -> i32 {
            self.complex_length()
        }
    ...
    impl BuiltInDataType for MockFloatComplexDataType {
    ...
    impl AbstractComplexDataType for MockFloatComplexDataType {
        fn float_type(&self) -> &dyn AbstractFloatDataType {
            &self.float_type
        }
    }

    impl FloatComplexDataType for MockFloatComplexDataType {
        fn float_complex_clone(&self, dtm: Option<Box<dyn DataTypeManager>>) -> Box<dyn FloatComplexDataType> {
            match dtm {
                None => Box::new(MockFloatComplexDataType {
                    float_type: MockFloat { length: self.float_type.length },
                    dtm_tag: self.dtm_tag,
                }),
                Some(_) => Box::new(MockFloatComplexDataType {
                    float_type: MockFloat { length: self.float_type.length },
    ...
    fn complex_floatcomplex() -> MockFloatComplexDataType {
        MockFloatComplexDataType { float_type: MockFloat { length: 4 }, dtm_tag: None }
    }
    ...
    fn usable_as_trait_object() {
        ...
        let dyn_dt: &dyn FloatComplexDataType = &dt;
        assert_eq!(dyn_dt.complex_length(), 8);
        assert_eq!(DataType::get_length(dyn_dt), 8);
        assert_eq!(dyn_dt.get_name(), "floatcomplex");
    }
    ...
    fn description_names_component_float_type() {
        ...
        assert_eq!(
            ...
            "The data type for a complex number: a + bi; consisting of two float values"
        );
    }
    ...
    fn clone_with_no_manager_preserves_identity_tag() {
        let dt = MockFloatComplexDataType { float_type: MockFloat { length: 4 }, dtm_tag: Some("mgr-a") };
        let cloned = dt.float_complex_clone(None);
    ...
    fn clone_with_new_manager_rebinds_instance() {
        let dt = MockFloatComplexDataType { float_type: MockFloat { length: 4 }, dtm_tag: Some("mgr-a") };
        let cloned = dt.float_complex_clone(Some(Box::new(MockDataTypeManager)));
        assert_eq!(cloned.complex_length(), 8);
    }
}
