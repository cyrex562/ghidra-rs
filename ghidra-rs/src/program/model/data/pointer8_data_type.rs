//! Port of `ghidra.program.model.data.Pointer8DataType`.
//!
...
//! this crate, `Pointer8DataType` contributes **no method overrides at all** -- its entire body
//! is two constructors (`Pointer8DataType()`/`Pointer8DataType(DataType dt)`, both forwarding to
//! `super(dt, 1)`, fixing the pointer length at 1 byte) plus a `static { ClassTranslator.put(...) }`
//! registration of the legacy class name `ghidra.program.model.data.Pointer8`.
//!
//! Neither is representable on a trait: traits cannot declare constructors or store fields (so the
//! fixed-length-1 behavior has nowhere to live except
//! [`PointerDataType::stored_length`](crate::program::model::data::pointer_data_type::PointerDataType::stored_length),
//! which a concrete implementation must simply initialize to [`POINTER8_LENGTH`]), and
...
//! `stored_length()` fixed at [`POINTER8_LENGTH`].
//!
...
/// Fixed pointer length (in bytes) for a [`Pointer8DataType`], standing in for the literal `1`
/// passed to the `PointerDataType(DataType, int)` superclass constructor by both
/// `Pointer8DataType` constructors.
pub const POINTER8_LENGTH: i32 = 1;

/// Pointer8 is really a factory for generating 1-byte pointers.
///
/// Port of `ghidra.program.model.data.Pointer8DataType`. See the module-level documentation for
/// why this trait adds no new methods beyond what
/// [`PointerDataType`] already provides.
pub trait Pointer8DataType: PointerDataType {}
...
mod tests {
    ...
    impl DataOrganization for MockDataOrganization {
        ...
        fn get_pointer_size(&self) -> i32 {
            // Deliberately different from POINTER8_LENGTH, so tests can confirm the *stored*
            // fixed length (1) wins over whatever the data organization's native pointer size is.
            8
        }
    ...
    struct MockPointer8DataType {
        referenced: Option<Box<dyn DataType>>,
    }

    impl DataType for MockPointer8DataType {
    ...
    impl BuiltInDataType for MockPointer8DataType {
    ...
    impl Pointer for MockPointer8DataType {
    ...
    impl PointerDataType for MockPointer8DataType {
        ...
        fn set_stored_length(&mut self, _length: i32) {
            // Pointer8DataType's length is always fixed at POINTER8_LENGTH; a real
    ...
    impl Pointer8DataType for MockPointer8DataType {}
    ...
    fn usable_as_trait_object() {
        let dt = MockPointer8DataType { referenced: None };
        let dyn_dt: &dyn Pointer8DataType = &dt;
        assert_eq!(dyn_dt.stored_length(), POINTER8_LENGTH);
    }

    #[test]
    fn length_is_fixed_at_1_byte_regardless_of_data_organization_pointer_size() {
        let dt = MockPointer8DataType { referenced: None };
        // The mock DataOrganization reports an 8-byte native pointer size, but a Pointer8DataType
        // always reports 1 -- exactly mirroring `super(dt, 1)` fixing the Java `length` field.
        assert_eq!(DataType::get_length(&dt), POINTER8_LENGTH);
        assert_eq!(dt.get_data_organization().get_pointer_size(), 8);
    }

    #[test]
    fn display_name_reflects_pointer_arithmetic_over_stored_length() {
        let untyped = MockPointer8DataType { referenced: None };
        assert_eq!(untyped.pointer_data_type_impl_display_name(), "pointer8");

        let typed = MockPointer8DataType {
