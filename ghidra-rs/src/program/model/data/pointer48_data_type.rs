//! Port of `ghidra.program.model.data.Pointer48DataType`.
//!
...
//! this crate, `Pointer48DataType` contributes **no method overrides at all** -- its entire body
//! is two constructors (`Pointer48DataType()`/`Pointer48DataType(DataType dt)`, both forwarding to
//! `super(dt, 6)`, fixing the pointer length at 6 bytes) plus a `static { ClassTranslator.put(...) }`
//! registration of the legacy class name `ghidra.program.model.data.Pointer48`.
//!
//! Neither is representable on a trait: traits cannot declare constructors or store fields (so the
//! fixed-length-6 behavior has nowhere to live except
//! [`PointerDataType::stored_length`](crate::program::model::data::pointer_data_type::PointerDataType::stored_length),
//! which a concrete implementation must simply initialize to [`POINTER48_LENGTH`]), and
...
//! `stored_length()` fixed at [`POINTER48_LENGTH`].
//!
...
/// Fixed pointer length (in bytes) for a [`Pointer48DataType`], standing in for the literal `6`
/// passed to the `PointerDataType(DataType, int)` superclass constructor by both
/// `Pointer48DataType` constructors.
pub const POINTER48_LENGTH: i32 = 6;

/// Pointer48 is really a factory for generating 6-byte pointers.
///
/// Port of `ghidra.program.model.data.Pointer48DataType`. See the module-level documentation for
/// why this trait adds no new methods beyond what
/// [`PointerDataType`] already provides.
pub trait Pointer48DataType: PointerDataType {}
...
mod tests {
    ...
    impl DataOrganization for MockDataOrganization {
        ...
        fn get_pointer_size(&self) -> i32 {
            // Deliberately different from POINTER48_LENGTH, so tests can confirm the *stored*
            // fixed length (6) wins over whatever the data organization's native pointer size is.
            8
        }
    ...
    struct MockPointer48DataType {
        referenced: Option<Box<dyn DataType>>,
    }

    impl DataType for MockPointer48DataType {
    ...
    impl BuiltInDataType for MockPointer48DataType {
    ...
    impl Pointer for MockPointer48DataType {
    ...
    impl PointerDataType for MockPointer48DataType {
        ...
        fn set_stored_length(&mut self, _length: i32) {
            // Pointer48DataType's length is always fixed at POINTER48_LENGTH; a real
    ...
    impl Pointer48DataType for MockPointer48DataType {}
    ...
    fn usable_as_trait_object() {
        let dt = MockPointer48DataType { referenced: None };
        let dyn_dt: &dyn Pointer48DataType = &dt;
        assert_eq!(dyn_dt.stored_length(), POINTER48_LENGTH);
    }

    #[test]
    fn length_is_fixed_at_6_bytes_regardless_of_data_organization_pointer_size() {
        let dt = MockPointer48DataType { referenced: None };
        // The mock DataOrganization reports an 8-byte native pointer size, but a Pointer48DataType
        // always reports 6 -- exactly mirroring `super(dt, 6)` fixing the Java `length` field.
        assert_eq!(DataType::get_length(&dt), POINTER48_LENGTH);
        assert_eq!(dt.get_data_organization().get_pointer_size(), 8);
    }

    #[test]
    fn display_name_reflects_pointer_arithmetic_over_stored_length() {
        let untyped = MockPointer48DataType { referenced: None };
        assert_eq!(untyped.pointer_data_type_impl_display_name(), "pointer48");

        let typed = MockPointer48DataType {
