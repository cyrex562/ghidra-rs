//! Port of `ghidra.program.model.data.Complex16DataType`.
//!
//! Provides a definition of a `complex` built-in data type consisting of two 64-bit floating
//! point numbers in the IEEE 754 double precision format.
//!
//! Port of `ghidra.program.model.data.Complex16DataType`, promoted straight to a trait because it
//! was selected as a dependency-cycle cut-point.
//!
//! The Java class `extends AbstractComplexDataType`, already ported as a trait
//! ([`AbstractComplexDataType`]). Every method `Complex16DataType` overrides beyond its
...
//!
//! The constructor (`super("complex16", Float8DataType.dataType, dtm)`) has no trait equivalent
...
//! [`Float8DataType`](crate::program::model::data::float8_data_type::Float8DataType)-shaped
...
pub trait Complex16DataType: AbstractComplexDataType {
    /// Returns an instance of this DataType using the specified `DataTypeManager` to allow its
    /// use of the corresponding `DataOrganization` while retaining its unique identity.
    ///
    /// Port of `Complex16DataType.clone(DataTypeManager)`, which overrides
    ...
    /// [`Float8DataType::float8_clone`](crate::program::model::data::float8_data_type::Float8DataType::float8_clone).
    fn complex16_clone(&self, dtm: Option<Box<dyn DataTypeManager>>) -> Box<dyn Complex16DataType>;
}
...
mod tests {
    ...
    struct MockComplex16DataType {
        float_type: MockFloat,
        dtm_tag: Option<&'static str>,
    }

    impl DataType for MockComplex16DataType {
        fn get_name(&self) -> String {
            "complex16".to_string()
        }
        fn get_length(&self) -> i32 {
    ...
    impl BuiltInDataType for MockComplex16DataType {
    ...
    impl AbstractComplexDataType for MockComplex16DataType {
        fn float_type(&self) -> &dyn AbstractFloatDataType {
            &self.float_type
        }
    }

    impl Complex16DataType for MockComplex16DataType {
        fn complex16_clone(&self, dtm: Option<Box<dyn DataTypeManager>>) -> Box<dyn Complex16DataType> {
            match dtm {
                None => Box::new(MockComplex16DataType {
                    float_type: MockFloat { length: self.float_type.length },
                    dtm_tag: self.dtm_tag,
                }),
                Some(_) => Box::new(MockComplex16DataType {
                    float_type: MockFloat { length: self.float_type.length },
    ...
    fn complex16() -> MockComplex16DataType {
        MockComplex16DataType { float_type: MockFloat { length: 8 }, dtm_tag: None }
    }
    ...
    fn description_names_component_float_type() {
        ...
        assert_eq!(
            ...
            "The data type for a complex number: a + bi; consisting of two float8 values"
        );
    }
    ...
    fn complex_value_decodes_real_and_imaginary_from_adjacent_halves() {
        ...
        bytes.extend(vec![0, 0, 0, 0, 0, 0, 0, 7]);
    ...
    fn clone_with_no_manager_preserves_identity_tag() {
        let dt = MockComplex16DataType { float_type: MockFloat { length: 8 }, dtm_tag: Some("mgr-a") };
        let cloned = dt.complex16_clone(None);
    ...
    fn clone_with_new_manager_rebinds_instance() {
        let dt = MockComplex16DataType { float_type: MockFloat { length: 8 }, dtm_tag: Some("mgr-a") };
        let cloned = dt.complex16_clone(Some(Box::new(MockDataTypeManager)));
        assert_eq!(cloned.complex_length(), 16);
    }
}
