//! Port of `ghidra.program.model.data.Pointer32DataType`.
//!
...
//! this crate, `Pointer32DataType` contributes **no method overrides at all** -- its entire body
//! is two constructors (`Pointer32DataType()`/`Pointer32DataType(DataType dt)`, both forwarding to
//! `super(dt, 4)`, fixing the pointer length at 4 bytes) plus a `static { ClassTranslator.put(...) }`
//! registration of the legacy class name `ghidra.program.model.data.Pointer32`.
//!
//! Neither is representable on a trait: traits cannot declare constructors or store fields (so the
//! fixed-length-4 behavior has nowhere to live except
//! [`PointerDataType::stored_length`](crate::program::model::data::pointer_data_type::PointerDataType::stored_length),
//! which a concrete implementation must simply initialize to [`POINTER32_LENGTH`]), and
...
//! `stored_length()` fixed at [`POINTER32_LENGTH`].
//!
...
/// Fixed pointer length (in bytes) for a [`Pointer32DataType`], standing in for the literal `4`
/// passed to the `PointerDataType(DataType, int)` superclass constructor by both
/// `Pointer32DataType` constructors.
pub const POINTER32_LENGTH: i32 = 4;

/// Pointer32 is really a factory for generating 4-byte pointers.
///
/// Port of `ghidra.program.model.data.Pointer32DataType`. See the module-level documentation for
/// why this trait adds no new methods beyond what
/// [`PointerDataType`] already provides.
pub trait Pointer32DataType: PointerDataType {}
...
mod tests {
    ...
    impl DataOrganization for MockDataOrganization {
        ...
        fn get_pointer_size(&self) -> i32 {
            // Deliberately different from POINTER32_LENGTH, so tests can confirm the *stored*
            // fixed length (4) wins over whatever the data organization's native pointer size is.
            8
        }
    ...
    struct MockPointer32DataType {
        referenced: Option<Box<dyn DataType>>,
    }

    impl DataType for MockPointer32DataType {
    ...
    impl BuiltInDataType for MockPointer32DataType {
    ...
    impl Pointer for MockPointer32DataType {
    ...
    impl PointerDataType for MockPointer32DataType {
        ...
        fn set_stored_length(&mut self, _length: i32) {
            // Pointer32DataType's length is always fixed at POINTER32_LENGTH; a real
    ...
    impl Pointer32DataType for MockPointer32DataType {}
    ...
    fn usable_as_trait_object() {
        let dt = MockPointer32DataType { referenced: None };
        let dyn_dt: &dyn Pointer32DataType = &dt;
        assert_eq!(dyn_dt.stored_length(), POINTER32_LENGTH);
    }

    #[test]
    fn length_is_fixed_at_4_bytes_regardless_of_data_organization_pointer_size() {
        let dt = MockPointer32DataType { referenced: None };
        // The mock DataOrganization reports an 8-byte native pointer size, but a Pointer32DataType
        // always reports 4 -- exactly mirroring `super(dt, 4)` fixing the Java `length` field.
        assert_eq!(DataType::get_length(&dt), POINTER32_LENGTH);
        assert_eq!(dt.get_data_organization().get_pointer_size(), 8);
    }

    #[test]
    fn display_name_reflects_pointer_arithmetic_over_stored_length() {
        let untyped = MockPointer32DataType { referenced: None };
        assert_eq!(untyped.pointer_data_type_impl_display_name(), "pointer32");

        let typed = MockPointer32DataType {
