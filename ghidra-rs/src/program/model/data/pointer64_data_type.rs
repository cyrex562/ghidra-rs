//! Port of `ghidra.program.model.data.Pointer64DataType`.
//!
...
//! this crate, `Pointer64DataType` contributes **no method overrides at all** -- its entire body
//! is two constructors (`Pointer64DataType()`/`Pointer64DataType(DataType dt)`, both forwarding to
//! `super(dt, 8)`, fixing the pointer length at 8 bytes) plus a `static { ClassTranslator.put(...) }`
//! registration of the legacy class name `ghidra.program.model.data.Pointer64`.
//!
//! Neither is representable on a trait: traits cannot declare constructors or store fields (so the
//! fixed-length-8 behavior has nowhere to live except
//! [`PointerDataType::stored_length`](crate::program::model::data::pointer_data_type::PointerDataType::stored_length),
//! which a concrete implementation must simply initialize to [`POINTER64_LENGTH`]), and
...
//! `stored_length()` fixed at [`POINTER64_LENGTH`].
//!
...
/// Fixed pointer length (in bytes) for a [`Pointer64DataType`], standing in for the literal `8`
/// passed to the `PointerDataType(DataType, int)` superclass constructor by both
/// `Pointer64DataType` constructors.
pub const POINTER64_LENGTH: i32 = 8;

/// Pointer64 is really a factory for generating 8-byte pointers.
///
/// Port of `ghidra.program.model.data.Pointer64DataType`. See the module-level documentation for
/// why this trait adds no new methods beyond what
/// [`PointerDataType`] already provides.
pub trait Pointer64DataType: PointerDataType {}
...
mod tests {
    ...
    impl DataOrganization for MockDataOrganization {
        ...
        fn get_pointer_size(&self) -> i32 {
            // Deliberately different from POINTER64_LENGTH, so tests can confirm the *stored*
            // fixed length (8) wins over whatever the data organization's native pointer size is.
            8
        }
    ...
    struct MockPointer64DataType {
        referenced: Option<Box<dyn DataType>>,
    }

    impl DataType for MockPointer64DataType {
    ...
    impl BuiltInDataType for MockPointer64DataType {
    ...
    impl Pointer for MockPointer64DataType {
    ...
    impl PointerDataType for MockPointer64DataType {
        ...
        fn set_stored_length(&mut self, _length: i32) {
            // Pointer64DataType's length is always fixed at POINTER64_LENGTH; a real
    ...
    impl Pointer64DataType for MockPointer64DataType {}
    ...
    fn usable_as_trait_object() {
        let dt = MockPointer64DataType { referenced: None };
        let dyn_dt: &dyn Pointer64DataType = &dt;
        assert_eq!(dyn_dt.stored_length(), POINTER64_LENGTH);
    }

    #[test]
    fn length_is_fixed_at_8_bytes_regardless_of_data_organization_pointer_size() {
        let dt = MockPointer64DataType { referenced: None };
        // The mock DataOrganization reports an 8-byte native pointer size, but a Pointer64DataType
        // always reports 8 -- exactly mirroring `super(dt, 8)` fixing the Java `length` field.
        assert_eq!(DataType::get_length(&dt), POINTER64_LENGTH);
        assert_eq!(dt.get_data_organization().get_pointer_size(), 8);
    }

    #[test]
    fn display_name_reflects_pointer_arithmetic_over_stored_length() {
        let untyped = MockPointer64DataType { referenced: None };
        assert_eq!(untyped.pointer_data_type_impl_display_name(), "pointer64");

        let typed = MockPointer64DataType {
