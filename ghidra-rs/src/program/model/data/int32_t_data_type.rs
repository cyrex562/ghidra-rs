...
use crate::program::seam_stubs::UInt32TDataType as UInt32TDataTypeStub;

/// 32-bit signed integer (C99 Standard type `int32_t`).
///
/// Port of `ghidra.program.model.data.Int32TDataType`, promoted straight to a trait because it
/// was selected as a dependency-cycle cut-point.
///
/// The Java class `extends AbstractSignedIntegerDataType`, itself a subclass of
/// `AbstractIntegerDataType`. Neither is yet ported, but every member `Int32TDataType` actually
...
///
/// Methods that only *override* an already-ported supertrait method with Int32TDataType-specific
...
/// real Int32TDataType-specific values for those overrides are exposed here under distinct
/// `int32_t_*` names; a future concrete implementation (once
...
pub trait Int32TDataType: DataType + BuiltInDataType {
    /// Determine if this type is signed.
    ///
    /// Port of the `final` `AbstractSignedIntegerDataType.isSigned()`, which `Int32TDataType`
    /// inherits unchanged.
    ...
    ///
    /// Port of `Int32TDataType.getLength()`, which overrides the abstract `DataType.getLength()`.
    /// Exposed under a distinct name since [`DataType`] already declares `get_length`; see the
    /// module docs for why it cannot be redeclared here. Always `4`, matching the fixed C99
    /// `int32_t` width (unlike `IntegerDataType`, this is not derived from a `DataOrganization`).
    fn int32_t_length(&self) -> i32 {
        4
    }

    /// A brief description of this data-type.
    ///
    /// Port of `Int32TDataType.getDescription()`, which overrides the abstract
    ...
    fn int32_t_description(&self) -> String {
        "Signed 32-bit Integer".to_string()
    }

    /// The C style data-type declaration for this data-type.
    ///
    /// Port of `Int32TDataType.getCDeclaration()`, which overrides
    ...
    /// 32-bit integer type).
    ///
    /// Port of `Int32TDataType.getOppositeSignednessDataType()`, which overrides the abstract
    /// `AbstractIntegerDataType.getOppositeSignednessDataType()`. Left as a required method (no
    /// default) since the real implementation clones the `UInt32TDataType.dataType` singleton,
    /// which is not ported yet (see [`UInt32TDataTypeStub`] placeholder).
    fn get_opposite_signedness_data_type(&self) -> Box<dyn UInt32TDataTypeStub>;

    /// Returns an instance of this DataType using the specified `DataTypeManager` to allow its
    /// use of the corresponding `DataOrganization` while retaining its unique identity.
    ///
    /// Port of `Int32TDataType.clone(DataTypeManager)`, which overrides
    ...
    fn int32_t_clone(&self, dtm: Option<Box<dyn DataTypeManager>>) -> Box<dyn Int32TDataType>;

    /// The C style data-type declaration for this data-type, given a specific `DataOrganization`.
    ///
    /// Port of `Int32TDataType.getCTypeDeclaration(DataOrganization)`, which overrides the
    ...
    /// (`int32_t`) after the organization's signed 32-bit C-type approximation.
    fn int32_t_get_c_type_declaration(&self, data_organization: &dyn DataOrganization) -> Option<String> {
        Some(format!(
            "typedef {}    int32_t;",
...
mod tests {
    ...
    impl UInt32TDataTypeStub for MockUInt32TDataType {}
    ...
    impl BuiltInDataType for MockInt32TDataType {
        fn get_c_type_declaration(
            ...
            data_organization.and_then(|org| self.int32_t_get_c_type_declaration(org))
        }
    ...
    impl Int32TDataType for MockInt32TDataType {
        fn get_opposite_signedness_data_type(&self) -> Box<dyn UInt32TDataTypeStub> {
            Box::new(MockUInt32TDataType)
        }

        fn int32_t_clone(&self, dtm: Option<Box<dyn DataTypeManager>>) -> Box<dyn Int32TDataType> {
            // Mirrors `Int32TDataType.clone(DataTypeManager)`: return an equivalent instance tied
            // to the requested manager (`None` here stands in for "already matches").
            match dtm {
                None => Box::new(MockInt32TDataType { dtm_tag: self.dtm_tag }),
                Some(_) => Box::new(MockInt32TDataType {
    ...
    fn usable_as_trait_object() {
        let dt = MockInt32TDataType { dtm_tag: None };
        let dyn_dt: &dyn Int32TDataType = &dt;
        assert!(dyn_dt.is_signed());
        assert_eq!(dyn_dt.int32_t_length(), 4);
        assert_eq!(dyn_dt.int32_t_description(), "Signed 32-bit Integer");
        assert_eq!(dyn_dt.get_c_declaration(), None);
        // The DataType supertrait's own get_name is independently reachable and untouched.
        assert_eq!(DataType::get_name(dyn_dt), "int32_t");
    }

    #[test]
    fn c_type_declaration_uses_signed_32bit_approximation() {
        let dt = MockInt32TDataType { dtm_tag: None };
        let org = MockDataOrganization;
        assert_eq!(
            dt.int32_t_get_c_type_declaration(&org),
            Some("typedef int32    int32_t;".to_string())
        );
    }

    #[test]
    fn opposite_signedness_returns_distinct_placeholder() {
        let dt = MockInt32TDataType { dtm_tag: Some("a") };
    ...
    fn clone_with_no_manager_preserves_identity_tag() {
        let dt = MockInt32TDataType { dtm_tag: Some("mgr-a") };
        let cloned = dt.int32_t_clone(None);
        assert_eq!(cloned.int32_t_description(), dt.int32_t_description());
        assert_eq!(cloned.int32_t_length(), 4);
    }

    #[test]
    fn clone_with_new_manager_rebinds_instance() {
        let dt = MockInt32TDataType { dtm_tag: Some("mgr-a") };
        let cloned = dt.int32_t_clone(Some(Box::new(MockDataTypeManager)));
