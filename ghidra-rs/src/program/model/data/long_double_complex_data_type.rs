//! Port of `ghidra.program.model.data.LongDoubleComplexDataType`.
//!
//! Provides a definition of a `complex` built-in data type consisting of two long double
//! point numbers in the IEEE 754 double precision format.
//!
//! The size of the long double numbers is determined by the program's data organization as
//! defined by the language/compiler spec (unlike the fixed-size `Complex8`/`Complex16`/`Complex32`
//! siblings).
//!
//! Port of `ghidra.program.model.data.LongDoubleComplexDataType`, promoted straight to a trait because it
//! was selected as a dependency-cycle cut-point.
//!
//! The Java class `extends AbstractComplexDataType`, already ported as a trait
//! ([`AbstractComplexDataType`]). Every method `LongDoubleComplexDataType` overrides beyond its
...
//!
//! The constructor (`super("longdoublecomplex", LongDoubleDataType.dataType, dtm)`) has no trait equivalent
...
//! [`LongDoubleDataType`](crate::program::model::data::long_double_data_type::LongDoubleDataType)-shaped
...
pub trait LongDoubleComplexDataType: AbstractComplexDataType {
    /// Returns an instance of this DataType using the specified `DataTypeManager` to allow its
    /// use of the corresponding `DataOrganization` while retaining its unique identity.
    ///
    /// Port of `LongDoubleComplexDataType.clone(DataTypeManager)`, which overrides
    ...
    /// [`LongDoubleDataType`](crate::program::model::data::long_double_data_type::LongDoubleDataType), which computes its own
    /// encoded length from the associated `DataOrganization` at construction time rather than a
    /// fixed constant.
    fn long_double_complex_clone(&self, dtm: Option<Box<dyn DataTypeManager>>) -> Box<dyn LongDoubleComplexDataType>;
}
...
mod tests {
    ...
    struct MockLongDoubleComplexDataType {
        float_type: MockFloat,
        dtm_tag: Option<&'static str>,
    }

    impl DataType for MockLongDoubleComplexDataType {
        fn get_name(&self) -> String {
            "longdoublecomplex".to_string()
        }
        fn get_length(&self) -> i32 {
            self.complex_length()
        }
    ...
    impl BuiltInDataType for MockLongDoubleComplexDataType {
    ...
    impl AbstractComplexDataType for MockLongDoubleComplexDataType {
        fn float_type(&self) -> &dyn AbstractFloatDataType {
            &self.float_type
        }
    }

    impl LongDoubleComplexDataType for MockLongDoubleComplexDataType {
        fn long_double_complex_clone(&self, dtm: Option<Box<dyn DataTypeManager>>) -> Box<dyn LongDoubleComplexDataType> {
            match dtm {
                None => Box::new(MockLongDoubleComplexDataType {
                    float_type: MockFloat { length: self.float_type.length },
                    dtm_tag: self.dtm_tag,
                }),
                Some(_) => Box::new(MockLongDoubleComplexDataType {
                    float_type: MockFloat { length: self.float_type.length },
    ...
    fn complex_longdoublecomplex() -> MockLongDoubleComplexDataType {
        MockLongDoubleComplexDataType { float_type: MockFloat { length: 10 }, dtm_tag: None }
    }
    ...
    fn usable_as_trait_object() {
        let dt = complex_longdoublecomplex();
        let dyn_dt: &dyn LongDoubleComplexDataType = &dt;
        assert_eq!(dyn_dt.complex_length(), 20);
        assert_eq!(DataType::get_length(dyn_dt), 20);
        assert_eq!(dyn_dt.get_name(), "longdoublecomplex");
    }
    ...
    fn description_names_component_float_type() {
        ...
        assert_eq!(
            ...
            "The data type for a complex number: a + bi; consisting of two longdouble values"
        );
    }
    ...
    fn complex_value_decodes_real_and_imaginary_from_adjacent_halves() {
        ...
        // real 3.0, imaginary 7.0 as big-endian 10-byte IEEE 754 encodings
        let mut bytes = vec![0x40, 0x00, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
        bytes.extend(vec![0x40, 0x01, 0xe0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
        let buf = FixedMemBuffer(bytes);
        let value = dt.complex_value(&buf, &settings, 20).expect("decodes");
    ...
    fn clone_with_no_manager_preserves_identity_tag() {
        let dt = MockLongDoubleComplexDataType { float_type: MockFloat { length: 10 }, dtm_tag: Some("mgr-a") };
        let cloned = dt.long_double_complex_clone(None);
        assert_eq!(cloned.complex_length(), 20);
    }

    #[test]
    fn clone_with_new_manager_rebinds_instance() {
        let dt = MockLongDoubleComplexDataType { float_type: MockFloat { length: 10 }, dtm_tag: Some("mgr-a") };
        let cloned = dt.long_double_complex_clone(Some(Box::new(MockDataTypeManager)));
        assert_eq!(cloned.complex_length(), 20);
    }
}
