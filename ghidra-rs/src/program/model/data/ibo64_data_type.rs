//! Port of `ghidra.program.model.data.IBO64DataType`.
//!
//! `IBO64DataType` provides a Pointer-Typedef `BuiltIn` for a 64-bit Image Base Offset Relative
...
//! drops it (no `dataMgr` identity is tracked); [`IBO64DataType::clone_typedef`] always
//! constructs fresh.
...
/// Fixed pointer length (in bytes) for an [`IBO64DataType`].
pub const IBO64_LENGTH: i32 = 8;

/// Fixed name used by every [`IBO64DataType`] instance, standing in for the Java `NAME` constant.
pub const IBO64_NAME: &str = "ImageBaseOffset64";

/// 64-bit Image Base Offset Relative Pointer-Typedef.
///
/// Port of `ghidra.program.model.data.IBO64DataType`. See the module-level documentation for what
...
impl IBO64DataType {
    ...
    pub fn new() -> Self {
        let base = PointerTypedefBuiltInBase::new(Some(IBO64_NAME), None, IBO64_LENGTH)
            .expect("IBO64DataType's fixed construction arguments are always valid");
        let mut ibo = IBO64DataType { base };
    ...
    /// Port of `IBO64DataType.clone(DataTypeManager)`, always constructing fresh -- see the
    /// module docs on why the `dataMgr == dtm` short-circuit is dropped.
    pub fn clone_typedef(&self) -> IBO64DataType {
        IBO64DataType::new()
    }

    /// Create an IBO64 [`PointerTypedef`] with auto-naming. If needed, a name and category may be
    /// assigned to the returned instance. Unlike an immutable [`IBO64DataType`] instance, the
...
impl Default for IBO64DataType {
    fn default() -> Self {
        Self::new()
    }
}
...
impl DataType for IBO64DataType {
    ...
    fn get_description(&self) -> String {
        "64-bit Image Base Offset Relative Pointer-Typedef".to_string()
    }
    ...
    /// Narrowed to just the pointer-type setting, matching `IBO64DataType.getBuiltInSettingsDefinitions()`
    /// -- see the module docs.
    ...
    /// `dt`'s narrowed view, an asymmetry that made every two `IBO64DataType`s spuriously compare
    /// unequal.
...
impl TypeDef for IBO64DataType {
...
impl std::fmt::Display for IBO64DataType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "IBO64DataType: typedef {} {}",
...
mod tests {
    ...
    fn new_has_fixed_name_and_length() {
        let ibo = IBO64DataType::new();
        assert_eq!(DataType::get_name(&ibo), IBO64_NAME);
        assert_eq!(ibo.get_length(), IBO64_LENGTH);
        assert_eq!(DataType::get_description(&ibo), "64-bit Image Base Offset Relative Pointer-Typedef");
    }
    ...
    #[test]
    fn create_ibo64_pointer_typedef_wraps_referenced_type() {
        struct Leaf;
        ...
        let td = IBO64DataType::create_ibo64_pointer_typedef(Some(Box::new(Leaf)));
        assert_eq!(td.get_referenced_data_type().unwrap().get_name(), "Foo");
        assert_eq!(td.get_length(), IBO64_LENGTH);
