...
///
/// Stands in for `OverlapCodeSubModel.OVERLAP_MODEL_NAME`.
pub const NAME: &str = "Overlapped Code";

/// Model-O: defines subroutines with a unique entry point, which may share code with other
/// subroutines.
///
/// Each entry-point may either be a source or called entry-point and is identified using the
/// M-Model. This model defines the set of addresses contained within each subroutine based upon
/// the possible flows from its entry-point. Flows which encounter another entry-point are
/// terminated.
///
/// NOTE: This differs from the original definition of an entry point, however, the intent of the
/// O-Model is preserved.
///
/// Port of `ghidra.program.model.block.OverlapCodeSubModel`. The subroutine-discovery algorithm
/// (`getSubroutine`/`createSub`), the found-subroutine cache it relies on, and the `getModelM`
/// accessor are private/protected implementation details of the Java class rather than part of
...
pub trait OverlapCodeSubModel: SubroutineBlockModel {
...
use crate::program::model::listing::CommentType;
    ...
    /// [`OverlapCodeSubModel::get_listing`] rather than some other placeholder. Every other member
    /// is unreachable from this test and panics if called.
    ...
    /// The base subroutine model (the M-Model that `OverlapCodeSubModel` is built on top of --
    /// see `SubroutineBlockModel.getBaseSubroutineModel()`'s "generally the MultEntSubModel"
    /// contract). Each call to `get_base_subroutine_model` bumps a shared counter, and
    /// `get_listing` hands back a [`TaggedListing`] carrying its own `code_unit_count`, so the
    /// test below can prove that calling through a `Box<dyn OverlapCodeSubModel>` actually
    /// dispatches to this implementation's own state rather than just type-checking.
    struct OModel {
    ...
    fn get_listing_returns_this_models_own_listing_through_trait_object() {
        let model: Box<dyn OverlapCodeSubModel> = Box::new(OModel {
    ...
    fn name_constant_matches_java_display_name() {
        assert_eq!(NAME, "Overlapped Code");
    }
}
