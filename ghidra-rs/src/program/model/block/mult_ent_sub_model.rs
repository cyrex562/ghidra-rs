...
///
/// Stands in for `MultEntSubModel.NAME`.
pub const NAME: &str = "Multiple Entry";

/// Model-M: defines subroutines which do not share code with any other subroutine and may have
/// one or more entry points.
///
/// Each entry point represents either a source or called entry-point.
///
/// MODEL-M subroutines should be used to determine which subroutine(s) contains a particular
/// instruction. Since model-M subroutines yield the largest subroutines, they should be
/// particularly useful in the process of program slicing -- the process of splitting the program
/// into modules or subroutine cliques -- in order to begin to understand the structure and
/// functionality of the program.
///
/// Port of `ghidra.program.model.block.MultEntSubModel`. The subroutine-discovery algorithm
/// (`getAddressSetContaining`/`addDestinations`/`addSources`) and the found-subroutine cache it
/// relies on are private/protected implementation details of the Java class rather than part of
/// its public API, so they are not modeled here; only the public surface (plus the inherited
/// [`SubroutineBlockModel`]/`CodeBlockModel` contract) is captured.
pub trait MultEntSubModel: SubroutineBlockModel {
...
mod tests {
    ...
    /// [`MultEntSubModel::get_listing`] rather than some other placeholder. Every other member is
    ...
    /// The base subroutine model (`MultEntSubModel` is its own base -- see
    /// `SubroutineBlockModel.getBaseSubroutineModel()`'s "if there is no base model, this
    /// subroutine model is returned" contract). Each call to `get_base_subroutine_model` bumps a
    /// shared counter, and `get_listing` hands back a [`TaggedListing`] carrying its own
    /// `code_unit_count`, so the test below can prove that calling through a
    /// `Box<dyn MultEntSubModel>` actually dispatches to this implementation's own state rather
    ...
    impl MultEntSubModel for MModel {
        fn get_listing(&self) -> Arc<dyn Listing> {
    ...
    fn get_listing_returns_this_models_own_listing_through_trait_object() {
        let model: Box<dyn MultEntSubModel> = Box::new(MModel {
    ...
    #[test]
    fn base_subroutine_model_is_itself_and_bumps_shared_counter_on_each_call() {
