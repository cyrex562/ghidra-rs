...
///
/// Port of `ghidra.program.database.util.OrQuery`.
pub struct OrQuery {
    q1: Box<dyn Query>,
    q2: Box<dyn Query>,
}
...
impl Query for OrQuery {
    fn matches(&self, record: &DBRecord) -> bool {
        // Short-circuits like Java's `||`: q2 is not evaluated if q1 already matches.
...
mod tests {
    ...
    #[test]
    fn test_or_true_true_is_true() {
        let schema = make_schema();
        let record = DBRecord::new(schema, Field::Long(Some(1)));
        let query = OrQuery::new(Box::new(Constant(true)), Box::new(Constant(true)));
        assert!(query.matches(&record));
    }

    #[test]
    fn test_or_true_false_is_true() {
        let schema = make_schema();
        let record = DBRecord::new(schema, Field::Long(Some(1)));
        let query = OrQuery::new(Box::new(Constant(true)), Box::new(Constant(false)));
        assert!(query.matches(&record));
    }

    #[test]
    fn test_or_false_true_is_true() {
        let schema = make_schema();
        let record = DBRecord::new(schema, Field::Long(Some(1)));
        let query = OrQuery::new(Box::new(Constant(false)), Box::new(Constant(true)));
        assert!(query.matches(&record));
    }
    ...
    fn test_or_false_false_is_false() {
        let schema = make_schema();
        let record = DBRecord::new(schema, Field::Long(Some(1)));
        let query = OrQuery::new(Box::new(Constant(false)), Box::new(Constant(false)));
        assert!(!query.matches(&record));
    }

    /// Mirrors the Java doc: "If the first query matches, then the second query is not
    /// executed."
    ...
    #[test]
    fn test_or_short_circuits_when_first_query_matches() {
        let schema = make_schema();
        let record = DBRecord::new(schema, Field::Long(Some(1)));
        let second_called = Rc::new(Cell::new(false));
        let query = OrQuery::new(
            Box::new(Constant(true)),
