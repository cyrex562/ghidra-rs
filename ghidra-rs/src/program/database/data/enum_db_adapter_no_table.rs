//! Port of `ghidra.program.database.data.EnumDBAdapterNoTable`.
//!
//! Adapter needed for a read-only version of a data type manager that is not going to be
//! upgraded, and there is no Enumeration table in the data type manager.

use std::io;

use crate::framework::db::{DBHandle, DBRecord, Field, RecordIterator};
use crate::program::database::data::enum_db_adapter::EnumDBAdapter;
use crate::program::util::DBRecordAdapter;
use crate::util::UniversalID;

/// A `RecordIterator` that never yields any records, used in place of the unported
/// `ghidra.program.database.util.EmptyRecordIterator`. The Java method this stands in for
/// (`EnumDBAdapterNoTable.getRecords()`) actually returns `null`, but this trait's `get_records`
/// cannot represent that, so an always-empty iterator is used instead to preserve the "no
/// records" intent.
...
pub struct EnumDBAdapterNoTable;

impl EnumDBAdapterNoTable {
    /// `_handle` is the handle to the database which doesn't contain the table (unused).
    pub fn new(_handle: &DBHandle) -> Self {
        EnumDBAdapterNoTable
    }
}

impl DBRecordAdapter for EnumDBAdapterNoTable {
...
impl EnumDBAdapter for EnumDBAdapterNoTable {
    fn create_record(
        &mut self,
        _name: &str,
        ...
        Err(io::Error::new(
            ...
            "not allowed to update version prior to existence of the Enumeration Data Types table",
        ))
    }

    fn get_record(&self, _enum_id: i64) -> io::Result<Option<DBRecord>> {
        Ok(None)
    }
    ...
    fn update_record(&mut self, _record: &DBRecord, _set_last_change_time: bool) -> io::Result<()> {
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "no enumeration data types table exists",
        ))
    }

    fn remove_record(&mut self, _enum_id: i64) -> io::Result<bool> {
        Ok(false)
    }
...
mod tests {
    ...
    fn adapter() -> EnumDBAdapterNoTable {
        EnumDBAdapterNoTable::new(&DBHandle::new().unwrap())
    }
    ...
    fn create_and_update_record_are_unsupported() {
        ...
        assert_eq!(
            adapter
                .create_record("Colors", None, 5, 4, 0, 0, 0)
                .unwrap_err()
                .kind(),
            io::ErrorKind::Unsupported
        );
    ...
    fn behaves_as_trait_object() {
        let adapter: Box<dyn EnumDBAdapter> = Box::new(adapter());
        assert!(adapter.get_record(0).unwrap().is_none());
    }
}
