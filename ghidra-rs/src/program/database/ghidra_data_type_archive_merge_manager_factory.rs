...
/// Port of `ghidra.program.database.GhidraDataTypeArchiveMergeManagerFactory`.
///
/// The Java class overrides `DataTypeArchiveMergeManagerFactory.doGetMergeManager(...)` (a
/// pluggable-service factory hook) to build a `DataTypeArchiveMergeManager` for the four
/// `DomainObject`s (results/source/original/latest) involved in a data type archive merge,
/// casting `latestObj`/`sourceObj` to `DataTypeArchive` to pull their change sets. Modeled as a
/// trait (this class was selected as a cycle cut-point) so implementors can supply their own merge
/// manager construction without depending on the concrete `DataTypeArchiveMergeManager` (not yet
/// ported); the four `DomainObject` parameters mirror the overridden method's signature exactly
/// (the Java body's casts to `DataTypeManagerDomainObject`/`DataTypeArchive` are an implementation
/// detail of a given `do_get_merge_manager` impl, not part of the abstract contract).
pub trait GhidraDataTypeArchiveMergeManagerFactory {
...
mod tests {
    ...
    impl GhidraDataTypeArchiveMergeManagerFactory for MockFactory {
        fn do_get_merge_manager(
            &self,
            results_obj: Arc<dyn DomainObject>,
            source_obj: Arc<dyn DomainObject>,
    ...
    fn do_get_merge_manager_is_object_safe_and_receives_all_four_objects() {
        let factory: Box<dyn GhidraDataTypeArchiveMergeManagerFactory> = Box::new(MockFactory);
