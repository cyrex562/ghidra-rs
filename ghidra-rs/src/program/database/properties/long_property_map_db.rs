//! Port of `ghidra.program.database.properties.LongPropertyMapDB`.
//!
//! Property manager for `long`-valued properties, stored one value column per record. See
...
use crate::program::util::LongPropertyMap;
use crate::util::exception::NoValueException;

const VALUE_COL: usize = 0;

/// Database-backed long-valued property map.
///
/// Port of `ghidra.program.database.properties.LongPropertyMapDB`.
pub struct LongPropertyMapDB {
    state: DbObjectState,
    name: String,
...
impl LongPropertyMapDB {
    /// Construct a long property map, creating its underlying table in `db_handle` if it
    ...
    fn value_at(&self, addr: &Address) -> Option<i64> {
        self.table
            .read()
            .unwrap()
            ...
            .ok()
            .flatten()
            .and_then(|rec| rec.get_long(VALUE_COL))
    }
}

impl DbObject for LongPropertyMapDB {
...
impl LongPropertyMap for LongPropertyMapDB {
...
impl PropertyMap for LongPropertyMapDB {
    ...
    fn add(&mut self, addr: &Address, value: Option<Box<dyn Any>>) {
        match value {
            Some(v) => match v.downcast::<i64>() {
                Ok(i) => self.add_long(addr, *i),
                Err(_) => panic!("Long value required"),
            },
            None => {
                self.remove(addr);
            }
    ...
    fn move_range(&mut self, start: &Address, end: &Address, new_start: &Address) {
        let moved: Vec<(Address, i64)> = collect_sorted_addresses(&self.table.read().unwrap(), &self.space)
            .into_iter()
            .filter(|a| a >= start && a <= end)
        ...
        for (a, v) in moved {
            ...
            self.add_long(&new_addr, v);
        }
    }
}

impl PropertyMapDB for LongPropertyMapDB {
...
mod tests {
    ...
    fn map(name: &str) -> (DBHandle, LongPropertyMapDB) {
        let mut handle = DBHandle::new().unwrap();
        let m = LongPropertyMapDB::new(&mut handle, name, space()).unwrap();
        (handle, m)
    }

    #[test]
    fn add_long_and_get_long_round_trip() {
        let (_h, mut m) = map("ints");
        m.add_long(&addr(0x1000), 42);
        assert_eq!(m.get_long(&addr(0x1000)).unwrap(), 42);
        assert!(m.get_long(&addr(0x2000)).is_err());
    }

    #[test]
    fn add_with_dyn_any_dispatches_and_removes() {
        let (_h, mut m) = map("ints");
        m.add(&addr(0x1000), Some(Box::new(7i64)));
        assert_eq!(m.get_long(&addr(0x1000)).unwrap(), 7);
    ...
    #[test]
    #[should_panic(expected = "Long value required")]
    ...
    fn overwrite_updates_value() {
        let (_h, mut m) = map("ints");
        m.add_long(&addr(0x1000), 1);
        m.add_long(&addr(0x1000), 2);
        assert_eq!(m.get_long(&addr(0x1000)).unwrap(), 2);
    ...
    fn move_range_preserves_values() {
        let (_h, mut m) = map("ints");
        m.add_long(&addr(0x1000), 1);
        m.add_long(&addr(0x2000), 2);
        m.move_range(&addr(0x1000), &addr(0x2000), &addr(0x5000));

        assert!(!m.has_property(&addr(0x1000)));
        assert_eq!(m.get_long(&addr(0x5000)).unwrap(), 1);
        assert_eq!(m.get_long(&addr(0x6000)).unwrap(), 2);
    }
    ...
    #[test]
    fn get_value_class_is_i64() {
        let (_h, m) = map("ints");
        assert_eq!(m.get_value_class(), Some(TypeId::of::<i64>()));
    }
}
