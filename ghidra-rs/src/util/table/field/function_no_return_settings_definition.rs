...
const NORETURN: &str = "Show noreturn";
const NAME: &str = NORETURN;
const DESCRIPTION: &str = "On signals to show the noreturn function attribute when present";
const DEFAULT: bool = false;

/// Settings definition for showing the noreturn function attribute in tables.
///
/// Port of `ghidra.util.table.field.FunctionNoReturnSettingsDefinition`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub struct FunctionNoReturnSettingsDefinition;

impl FunctionNoReturnSettingsDefinition {
    /// The singleton instance of this settings definition.
    pub const DEF: FunctionNoReturnSettingsDefinition = FunctionNoReturnSettingsDefinition;
}

impl BooleanSettingsDefinition for FunctionNoReturnSettingsDefinition {
    fn get_value(&self, settings: &dyn Settings) -> bool {
        match settings.get_string(NORETURN) {
...
impl SettingsDefinition for FunctionNoReturnSettingsDefinition {
    ...
    fn clear(&self, settings: &mut dyn Settings) {
        settings.clear_setting(NORETURN);
    }

    fn copy_setting(&self, src_settings: &dyn Settings, dest_settings: &mut dyn Settings) {
        match src_settings.get_string(NORETURN) {
            Some(value) => dest_settings.set_string(NORETURN, &value),
            None => dest_settings.clear_setting(NORETURN),
        }
    }
}
...
mod tests {
    ...
    fn default_value_is_false_when_unset() {
        ...
        assert!(!FunctionNoReturnSettingsDefinition::DEF.get_value(&settings));
    }

    #[test]
    fn set_value_then_get_value_round_trips() {
        let mut settings = MockSettings::new();
        let def = FunctionNoReturnSettingsDefinition::DEF;
    ...
    fn name_storage_key_and_description_match_java_source() {
        let def = FunctionNoReturnSettingsDefinition::DEF;
        assert_eq!(def.get_name(), "Show noreturn");
        assert_eq!(def.get_storage_key(), "Show noreturn");
        assert_eq!(
            def.get_description(),
            "On signals to show the noreturn function attribute when present"
        );
    }
}
