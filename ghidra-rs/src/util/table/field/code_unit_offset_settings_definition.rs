...
const MEMORY_OFFSET: &str = "Code-unit offset";
const CHOICES: &[&str] = &[
...
/// Settings definition for the relative byte offset of a code unit to display in a table field.
///
/// Port of `ghidra.util.table.field.CodeUnitOffsetSettingsDefinition`.
...
impl CodeUnitOffsetSettingsDefinition {
    ...
    pub const DEF: CodeUnitOffsetSettingsDefinition = CodeUnitOffsetSettingsDefinition;
...
impl EnumSettingsDefinition for CodeUnitOffsetSettingsDefinition {
...
mod tests {
    ...
    fn default_choice_when_unset() {
        ...
        assert_eq!(
            CodeUnitOffsetSettingsDefinition::DEF.get_choice(&settings),
            DEFAULT_CHOICE
        );
    }
    ...
    fn default_choice_when_out_of_range() {
        ...
        let def = CodeUnitOffsetSettingsDefinition::DEF;
    ...
    fn name_storage_key_and_description_match_java_source() {
        let def = CodeUnitOffsetSettingsDefinition::DEF;
        assert_eq!(def.get_name(), "Code-unit offset");
        assert_eq!(def.get_storage_key(), "Code-unit offset");
        assert_eq!(
            def.get_description(),
