...
/// An array of `f64` (Java `double`) that grows as needed.
///
/// Port of `ghidra.util.datastruct.DoubleArray`. An index that was never
/// written (or was most recently `remove`d) reads back as `0.0`.
///
/// `DoubleArray.java` is a near-identical clone of `IntArray`/`LongArray`
/// with the element type swapped to `double`; this port follows the same
/// shape as [`super::int_array::IntArray`]/[`super::long_array::LongArray`].
/// Since `f64` has no total order/no `Eq`, this type derives `PartialEq`
...
impl DoubleArray {
    /// Creates a new, empty `DoubleArray`.
    pub fn new() -> Self {
        Self {
            doubles: vec![0.0; MIN_SIZE],
    ...
    pub fn put(&mut self, index: usize, value: f64) {
        ...
        if index >= self.doubles.len() {
            self.adjust_array(std::cmp::max(index as i64 + 1, self.doubles.len() as i64 * 2));
        }
    ...
    pub fn remove(&mut self, index: usize) {
        ...
        self.doubles[index] = 0.0;
        if index as i32 == self.last_non_zero_index {
            self.last_non_zero_index = self.find_last_non_zero_index();
        }
        if self.last_non_zero_index < self.doubles.len() as i32 / 4 {
    ...
    fn find_last_non_zero_index(&self) -> i32 {
        ...
        while i >= 0 {
            if self.doubles[i as usize] != 0.0 {
                return i;
            }
            i -= 1;
        }
...
impl Array for DoubleArray {
    fn remove(&mut self, index: usize) {
        DoubleArray::remove(self, index);
    }
    ...
    fn copy_data_to(&self, index: usize, table: &mut dyn DataTable, to_index: i32, to_col: i32) {
        table.put_double(to_index, to_col, self.get(index));
    }
}
...
mod tests {
    ...
    impl DataTable for MockDataTable {
        ...
        fn get_double(&self, row: i32, col: i32) -> f64 {
            *self.doubles.get(&(row, col)).unwrap_or(&0.0)
        }
    ...
    #[test]
    fn array_trait_copy_data_to_writes_double_column() {
        let mut arr = DoubleArray::new();
        arr.put(0, 99.5);
        let mut table = MockDataTable::default();
        Array::copy_data_to(&arr, 0, &mut table, 5, 1);
        assert_eq!(table.get_double(5, 1), 99.5);
    }
    ...
    fn default_matches_new() {
        assert_eq!(DoubleArray::default(), DoubleArray::new());
    }
}
