...
/// A hashtable with `i64` keys and `f64` values.
///
/// Port of `ghidra.util.datastruct.LongDoubleHashtable`.
#[derive(Debug, Clone)]
pub struct LongDoubleHashtable {
    indexer: LongKeyIndexer,
    values: Vec<f64>,
    capacity: i32,
}

impl LongDoubleHashtable {
    ...
    pub fn with_capacity(capacity: i32) -> Self {
        ...
        let values = vec![0.0; capacity as usize];
        Self {
            indexer,
            values,
            capacity,
    ...
    pub fn get(&self, key: i64) -> Result<f64, NoValueException> {
...
impl Default for LongDoubleHashtable {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    #[test]
    fn new_starts_empty() {
        let table = LongDoubleHashtable::new();
        assert_eq!(table.size(), 0);
    }

    #[test]
    fn put_and_get() {
        let mut table = LongDoubleHashtable::new();
        table.put(1, 1.5);
        assert_eq!(table.get(1), Ok(1.5));
    ...
    fn grows_beyond_initial_capacity_and_preserves_values() {
        let mut table = LongDoubleHashtable::with_capacity(2);
        for k in 0..50 {
            table.put(k, k as f64 * 1.5);
        }
        assert_eq!(table.size(), 50);
        for k in 0..50 {
            assert_eq!(table.get(k), Ok(k as f64 * 1.5));
        }
    }
    ...
    fn default_matches_new() {
        let table = LongDoubleHashtable::default();
        assert_eq!(table.size(), 0);
    }
}
