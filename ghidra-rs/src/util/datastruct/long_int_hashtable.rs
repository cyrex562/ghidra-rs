...
/// A hashtable with `i64` keys and `i32` values.
///
/// Port of `ghidra.util.datastruct.LongIntHashtable`.
#[derive(Debug, Clone)]
pub struct LongIntHashtable {
    indexer: LongKeyIndexer,
    values: Vec<i32>,
    capacity: i32,
}

impl LongIntHashtable {
    ...
    pub fn get(&self, key: i64) -> Result<i32, NoValueException> {
...
impl Default for LongIntHashtable {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    #[test]
    fn new_starts_empty() {
        let table = LongIntHashtable::new();
        assert_eq!(table.size(), 0);
    }

    #[test]
    fn put_and_get() {
        let mut table = LongIntHashtable::new();
        table.put(1, 15);
    ...
    fn grows_beyond_initial_capacity_and_preserves_values() {
        let mut table = LongIntHashtable::with_capacity(2);
        for k in 0..50 {
            table.put(k, k as i32 * 3);
        }
        assert_eq!(table.size(), 50);
        for k in 0..50 {
            assert_eq!(table.get(k), Ok(k as i32 * 3));
        }
    }
    ...
    fn default_matches_new() {
        let table = LongIntHashtable::default();
        assert_eq!(table.size(), 0);
    }
}
