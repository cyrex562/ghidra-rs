...
/// Manages multiple linked lists of `i32` indexes backed by a shared pool.
///
...
impl IntListIndexer {
    /// Creates a new `IntListIndexer` with `num_lists` lists and a pool of
    /// `capacity` index resources, all initially free.
    pub fn new(num_lists: i32, capacity: i32) -> Self {
    ...
    pub fn add(&mut self, list_id: i32) -> i32 {
    ...
    ///
    /// Panics if `list_id` is not in `[0, num_lists)`.
    pub fn append(&mut self, list_id: i32) -> i32 {
        if list_id < 0 || list_id >= self.num_lists {
            panic!("list_id out of bounds: {list_id}");
        }
        let index = self.allocate();
    ...
    /// `[0, capacity)`.
    pub fn remove(&mut self, list_id: i32, index: i32) {
        if list_id < 0 || list_id >= self.num_lists {
            panic!("list_id out of bounds: {list_id}");
        }
    ...
    pub fn remove_all(&mut self, list_id: i32) {
    ...
    ///
    /// Returns `-1` when the pool is already at `i32::MAX`.
    pub fn get_new_capacity(&self) -> i32 {
        if self.capacity == i32::MAX {
            return -1;
        }
        if self.capacity < i32::MAX / 2 {
            self.capacity * 2
        } else {
            i32::MAX
        }
    ...
    /// the list is empty.
    pub fn first(&self, list_id: i32) -> i32 {
    ...
    /// this is a no-op.
    pub fn grow_capacity(&mut self, new_capacity: i32) {
    ...
    pub fn grow_num_lists(&mut self, new_list_size: i32) {
    ...
    ///
    /// Panics if `list_id` is not in `[0, num_lists)`.
    pub fn get_list_size(&self, list_id: i32) -> i32 {
        if list_id < 0 || list_id >= self.num_lists {
            panic!("list_id out of bounds: {list_id}");
        }
        let mut count = 0i32;
...
mod tests {
    ...
    #[test]
    fn new_starts_empty() {
        let idx = IntListIndexer::new(3, 10);
    ...
    fn add_prepends_to_list() {
        let mut idx = IntListIndexer::new(1, 10);
        let a = idx.add(0);
        let b = idx.add(0);
        let c = idx.add(0);
    ...
    fn add_and_append_use_independent_lists() {
        let mut idx = IntListIndexer::new(2, 10);
        let a = idx.add(0);
        let b = idx.append(1);
        assert_eq!(idx.first(0), a);
    ...
    fn grow_num_lists_noop_when_not_larger() {
        let mut idx = IntListIndexer::new(5, 10);
    ...
    fn freed_indexes_are_reused() {
        ...
        let c = idx.add(0);
        assert!(c >= 0 && c < 4);
        // reused one of the previously freed indexes
        assert!(c == a || c == _b);
    }

    #[test]
    fn multiple_clear_cycles() {
        let mut idx = IntListIndexer::new(2, 4);
        for _ in 0..3 {
            idx.add(0);
            idx.add(0);
            idx.add(1);
    ...
    fn get_list_size_invalid_list_id_panics() {
        let idx = IntListIndexer::new(2, 10);
