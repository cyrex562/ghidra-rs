...
/// An array of `Vec<i16>` (Java `short[]`) that grows as needed.
///
/// Port of `ghidra.util.datastruct.ShortArrayArray`. Backed by
...
pub struct ShortArrayArray {
    inner: PackedArrayArray<i16>,
}

impl ShortArrayArray {
    /// Creates a new, empty `ShortArrayArray`.
    pub fn new() -> Self {
    ...
    pub fn put(&mut self, index: usize, value: Option<&[i16]>) {
        self.inner.put(index, value);
    }

    /// Returns a copy of the `i16` slice stored at `index`, or `None` if not
    /// initialized to another value.
    pub fn get(&self, index: usize) -> Option<Vec<i16>> {
...
impl Default for ShortArrayArray {
    fn default() -> Self {
        Self::new()
    }
}

impl Array for ShortArrayArray {
    fn remove(&mut self, index: usize) {
        ShortArrayArray::remove(self, index);
    }
    ...
    fn copy_data_to(&self, index: usize, table: &mut dyn DataTable, to_index: i32, to_col: i32) {
        table.put_short_array(to_index, to_col, self.get(index).unwrap_or_default());
    }
}
...
mod tests {
    ...
    impl DataTable for MockDataTable {
        ...
        fn put_short_array(&mut self, _row: i32, _col: i32, value: Vec<i16>) {
    ...
    fn put_and_get_roundtrip() {
        let mut arr = ShortArrayArray::new();
    ...
    #[test]
    fn array_trait_copy_data_to_writes_short_array_column() {
        let mut arr = ShortArrayArray::new();
        arr.put(0, Some(&[1, 2, 3]));
        let mut table = MockDataTable::default();
        Array::copy_data_to(&arr, 0, &mut table, 5, 1);
        assert_eq!(table.get_short_array(5, 1), vec![1, 2, 3]);
    }

    #[test]
    fn default_matches_new() {
        assert_eq!(ShortArrayArray::default(), ShortArrayArray::new());
    }
}
