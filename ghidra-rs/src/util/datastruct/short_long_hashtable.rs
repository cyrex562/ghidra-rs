...
/// A hashtable with `i16` keys and `i64` values.
///
/// Port of `ghidra.util.datastruct.ShortLongHashtable`.
#[derive(Debug, Clone)]
pub struct ShortLongHashtable {
    indexer: ShortKeyIndexer,
    values: Vec<i64>,
    capacity: i16,
}

impl ShortLongHashtable {
    ...
    pub fn get(&self, key: i16) -> Result<i64, NoValueException> {
...
impl Default for ShortLongHashtable {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    #[test]
    fn new_starts_empty() {
        let table = ShortLongHashtable::new();
        assert_eq!(table.size(), 0);
    }

    #[test]
    fn put_and_get() {
        let mut table = ShortLongHashtable::new();
        table.put(1, 15);
    ...
    fn grows_beyond_initial_capacity_and_preserves_values() {
        let mut table = ShortLongHashtable::with_capacity(2);
        for k in 0..50 {
            table.put(k, k as i64 * 3);
        }
        assert_eq!(table.size(), 50);
        for k in 0..50 {
            assert_eq!(table.get(k), Ok(k as i64 * 3));
        }
    }
    ...
    fn default_matches_new() {
        let table = ShortLongHashtable::default();
        assert_eq!(table.size(), 0);
    }
}
