...
/// Converts arbitrary `i64` keys into compacted `i32` indexes suitable for use
...
impl LongKeyIndexer {
    /// Creates a new `LongKeyIndexer` with a default capacity.
    pub fn new() -> Self {
        Self::with_capacity(DEFAULT_CAPACITY)
    }

    /// Creates a new `LongKeyIndexer` with a given initial capacity.
    ...
    /// allocated.
    pub fn put(&mut self, key: i64) -> i32 {
        ...
        if index == -1 {
            ...
            let hashcode = ((key as i32) & 0x7fffffff) % self.capacity;
    ...
    pub fn remove(&mut self, key: i64) -> i32 {
    ...
    pub fn get_keys(&self) -> Vec<i64> {
        ...
        if pos != self.get_size() as usize {
            panic!(
                "Trouble in LongKeyIndexer.get_keys(), size = {}  pos= {}",
                self.get_size(),
                pos
            );
        }
...
impl Default for LongKeyIndexer {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn with_capacity_rounds_up_to_prime() {
        let idx = LongKeyIndexer::with_capacity(10);
        assert_eq!(idx.get_capacity(), 17);
    }

    #[test]
    fn put_new_key_allocates_index() {
        let mut idx = LongKeyIndexer::new();
        let i = idx.put(42);
        assert!(i >= 0);
    ...
    fn grows_when_capacity_exceeded_and_preserves_keys() {
        let mut idx = LongKeyIndexer::with_capacity(2);
        let initial_capacity = idx.get_capacity();
        let mut indices = Vec::new();
        for k in 0..(initial_capacity as i64 * 3) {
    ...
    fn large_keys_beyond_i32_range_are_supported() {
        ...
        let i = idx.put(big_key);
        assert_eq!(idx.get(big_key), i);
    }

    #[test]
    fn default_matches_new() {
        let idx = LongKeyIndexer::default();
        assert_eq!(idx.get_capacity(), 17);
    }
}
