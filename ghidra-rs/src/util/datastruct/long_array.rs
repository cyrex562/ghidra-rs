...
/// An array of `i64` (Java `long`) that grows as needed.
///
/// Port of `ghidra.util.datastruct.LongArray`. An index that was never
...
impl LongArray {
    /// Creates a new, empty `LongArray`.
    pub fn new() -> Self {
        Self {
            longs: vec![0; MIN_SIZE],
    ...
    pub fn put(&mut self, index: usize, value: i64) {
        if value == 0 {
            self.remove(index);
            return;
        }
        ...
        if index >= self.longs.len() {
            self.adjust_array(std::cmp::max(index as i64 + 1, self.longs.len() as i64 * 2));
        }
    ...
    pub fn remove(&mut self, index: usize) {
        ...
        if self.last_non_zero_index < self.longs.len() as i32 / 4 {
...
impl Array for LongArray {
    fn remove(&mut self, index: usize) {
        LongArray::remove(self, index);
    }
    ...
    fn copy_data_to(&self, index: usize, table: &mut dyn DataTable, to_index: i32, to_col: i32) {
        table.put_long(to_index, to_col, self.get(index));
    }
}
...
mod tests {
    ...
    impl DataTable for MockDataTable {
        ...
        fn get_long(&self, row: i32, col: i32) -> i64 {
            *self.longs.get(&(row, col)).unwrap_or(&0)
        }
    ...
    fn put_and_get_roundtrip() {
        let mut arr = LongArray::new();
        arr.put(2, 42_000_000_000);
        assert_eq!(arr.get(2), 42_000_000_000);
    ...
    #[test]
    fn array_trait_copy_data_to_writes_long_column() {
        let mut arr = LongArray::new();
        arr.put(0, 99);
        let mut table = MockDataTable::default();
        Array::copy_data_to(&arr, 0, &mut table, 5, 1);
        assert_eq!(table.get_long(5, 1), 99);
    }
    ...
    fn default_matches_new() {
        assert_eq!(LongArray::default(), LongArray::new());
    }
}
