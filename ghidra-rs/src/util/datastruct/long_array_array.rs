...
/// An array of `Vec<i64>` (Java `long[]`) that grows as needed.
///
/// Port of `ghidra.util.datastruct.LongArrayArray`. Backed by
...
pub struct LongArrayArray {
    inner: PackedArrayArray<i64>,
}

impl LongArrayArray {
    /// Creates a new, empty `LongArrayArray`.
    pub fn new() -> Self {
    ...
    pub fn put(&mut self, index: usize, value: Option<&[i64]>) {
        self.inner.put(index, value);
    }

    /// Returns a copy of the `i64` slice stored at `index`, or `None` if not
...
impl Default for LongArrayArray {
    fn default() -> Self {
        Self::new()
    }
}

impl Array for LongArrayArray {
    fn remove(&mut self, index: usize) {
        LongArrayArray::remove(self, index);
    }
    ...
    fn copy_data_to(&self, index: usize, table: &mut dyn DataTable, to_index: i32, to_col: i32) {
        table.put_long_array(to_index, to_col, self.get(index).unwrap_or_default());
    }
}
...
mod tests {
    ...
    impl DataTable for MockDataTable {
        ...
        fn put_long_array(&mut self, _row: i32, _col: i32, value: Vec<i64>) {
    ...
    fn put_and_get_roundtrip() {
        let mut arr = LongArrayArray::new();
    ...
    #[test]
    fn array_trait_copy_data_to_writes_long_array_column() {
        let mut arr = LongArrayArray::new();
        arr.put(0, Some(&[1, 2, 3]));
        let mut table = MockDataTable::default();
        Array::copy_data_to(&arr, 0, &mut table, 5, 1);
        assert_eq!(table.get_long_array(5, 1), vec![1, 2, 3]);
    }

    #[test]
    fn default_matches_new() {
        assert_eq!(LongArrayArray::default(), LongArrayArray::new());
    }
}
