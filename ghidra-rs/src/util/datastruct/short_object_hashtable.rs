...
/// A hashtable with `i16` keys and arbitrary values.
///
/// Port of `ghidra.util.datastruct.ShortObjectHashtable`.
#[derive(Debug, Clone)]
pub struct ShortObjectHashtable<T> {
    indexer: ShortKeyIndexer,
    values: Vec<Option<T>>,
    capacity: i16,
}

impl<T> ShortObjectHashtable<T> {
    ...
    pub fn put(&mut self, key: i16, value: T) {
    ...
    pub fn remove(&mut self, key: i16) -> Option<T> {
...
impl<T> Default for ShortObjectHashtable<T> {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    #[test]
    fn new_starts_empty() {
        let table: ShortObjectHashtable<&str> = ShortObjectHashtable::new();
        assert_eq!(table.size(), 0);
    }

    #[test]
    fn put_and_get() {
        let mut table = ShortObjectHashtable::new();
        table.put(1, "one");
    ...
    fn remove_missing_key_returns_none() {
        let mut table: ShortObjectHashtable<&str> = ShortObjectHashtable::new();
    ...
    fn grows_beyond_initial_capacity_and_preserves_values() {
        let mut table = ShortObjectHashtable::with_capacity(2);
        for k in 0..50 {
            table.put(k, k * 10);
        }
    ...
    fn default_matches_new() {
        let table: ShortObjectHashtable<&str> = ShortObjectHashtable::default();
        assert_eq!(table.size(), 0);
    }
}
