...
/// An array of `Vec<f64>` (Java `double[]`) that grows as needed.
///
/// Port of `ghidra.util.datastruct.DoubleArrayArray`. Backed by
...
pub struct DoubleArrayArray {
    inner: PackedArrayArray<f64>,
}

impl DoubleArrayArray {
    /// Creates a new, empty `DoubleArrayArray`.
    pub fn new() -> Self {
    ...
    pub fn put(&mut self, index: usize, value: Option<&[f64]>) {
        self.inner.put(index, value);
    }

    /// Returns a copy of the `f64` slice stored at `index`, or `None` if not
...
impl Default for DoubleArrayArray {
    fn default() -> Self {
        Self::new()
    }
}

impl Array for DoubleArrayArray {
    fn remove(&mut self, index: usize) {
        DoubleArrayArray::remove(self, index);
    }
    ...
    fn copy_data_to(&self, index: usize, table: &mut dyn DataTable, to_index: i32, to_col: i32) {
        table.put_double_array(to_index, to_col, self.get(index).unwrap_or_default());
    }
}
...
mod tests {
    ...
    impl DataTable for MockDataTable {
        ...
        fn put_double_array(&mut self, _row: i32, _col: i32, value: Vec<f64>) {
    ...
    fn put_and_get_roundtrip() {
        let mut arr = DoubleArrayArray::new();
        arr.put(1, Some(&[1.5, 2.5, 3.5]));
        assert_eq!(arr.get(1), Some(vec![1.5, 2.5, 3.5]));
        assert_eq!(arr.get_last_non_empty_index(), 1);
    }
    ...
    fn put_grows_backing_storage_past_min_size() {
        let mut arr = DoubleArrayArray::new();
        arr.put(50, Some(&[7.0, 8.0]));
        assert_eq!(arr.get(50), Some(vec![7.0, 8.0]));
    ...
    fn remove_updates_last_non_empty_index_and_shrinks() {
        let mut arr = DoubleArrayArray::new();
        arr.put(1, Some(&[1.0]));
        arr.put(200, Some(&[2.0]));
        arr.remove(200);
        assert_eq!(arr.get_last_non_empty_index(), 1);
        arr.put(2, Some(&[9.0]));
        assert_eq!(arr.get(2), Some(vec![9.0]));
    }

    #[test]
    fn array_trait_copy_data_to_writes_double_array_column() {
        let mut arr = DoubleArrayArray::new();
        arr.put(0, Some(&[1.0, 2.0, 3.0]));
        let mut table = MockDataTable::default();
        Array::copy_data_to(&arr, 0, &mut table, 5, 1);
        assert_eq!(table.get_double_array(5, 1), vec![1.0, 2.0, 3.0]);
    }
}
