...
use super::short_list_indexer::ShortListIndexer;

const DEFAULT_CAPACITY: i16 = 13;

/// Converts arbitrary `i16` keys into compacted `i16` indexes suitable for use
...
/// [`ShortListIndexer`], where all values in a list share the same hash code.
...
#[derive(Debug, Clone)]
pub struct ShortKeyIndexer {
    keys: Vec<i16>,
    indexer: ShortListIndexer,
    capacity: i16,
}

impl ShortKeyIndexer {
    /// Creates a new `ShortKeyIndexer` with a default capacity.
    pub fn new() -> Self {
        Self::with_capacity(DEFAULT_CAPACITY)
    }

    /// Creates a new `ShortKeyIndexer` with a given initial capacity.
    pub fn with_capacity(capacity: i16) -> Self {
        let capacity = next_prime(capacity as i32) as i16;
        Self {
            keys: vec![0; capacity as usize],
            indexer: ShortListIndexer::new(capacity, capacity),
            capacity,
        }
    }
    ...
    /// allocated.
    pub fn put(&mut self, key: i16) -> i16 {
        ...
        if index == -1 {
            ...
            let hashcode = (key & 0x7fff) % self.capacity;
    ...
    pub fn remove(&mut self, key: i16) -> i16 {
    ...
    pub fn get_keys(&self) -> Vec<i16> {
        ...
        if pos != self.get_size() as usize {
            panic!(
                "Trouble in ShortKeyIndexer.get_keys(), size = {}  pos= {}",
                self.get_size(),
                pos
            );
        }
    ...
    /// we had to grow.
    fn grow(&mut self) {
        let new_capacity = next_prime(self.indexer.get_new_capacity() as i32) as i16;
...
impl Default for ShortKeyIndexer {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn with_capacity_rounds_up_to_prime() {
        let idx = ShortKeyIndexer::with_capacity(10);
        assert_eq!(idx.get_capacity(), 17);
    }

    #[test]
    fn put_new_key_allocates_index() {
        let mut idx = ShortKeyIndexer::new();
        let i = idx.put(42);
        assert!(i >= 0);
    ...
    fn grows_when_capacity_exceeded_and_preserves_keys() {
        let mut idx = ShortKeyIndexer::with_capacity(2);
