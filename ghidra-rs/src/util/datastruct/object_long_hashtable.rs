...
/// A hashtable with arbitrary (`Hash + Eq + Clone`) keys and `i64` values.
///
/// Port of `ghidra.util.datastruct.ObjectLongHashtable<T>`. `ObjectLongHashtable.java`
/// is a near-identical clone of `ObjectIntHashtable.java` with the value type
/// swapped from `int` to `long`; see [`super::object_int_hashtable::ObjectIntHashtable`]
/// for the sibling with the same shape. Java buckets keys by `key.hashCode()`;
/// since Rust has no runtime-reflective `hashCode()`, this is built on
/// [`ObjectKeyIndexer`], which requires `T: Hash + Eq` instead (see that
/// module's docs for the hashing caveat).
#[derive(Debug, Clone)]
pub struct ObjectLongHashtable<T> {
    indexer: ObjectKeyIndexer<T>,
    values: Vec<i64>,
    capacity: i32,
}

impl<T: Hash + Eq + Clone> ObjectLongHashtable<T> {
    ...
    pub fn put(&mut self, key: T, value: i64) {
    ...
    pub fn get(&self, key: &T) -> Result<i64, NoValueException> {
...
impl<T: Hash + Eq + Clone> Default for ObjectLongHashtable<T> {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    #[test]
    fn new_starts_empty() {
        let table: ObjectLongHashtable<String> = ObjectLongHashtable::new();
        assert_eq!(table.size(), 0);
    }

    #[test]
    fn put_and_get() {
        let mut table = ObjectLongHashtable::new();
        table.put("one".to_string(), 1_000_000_000_000);
        assert_eq!(table.get(&"one".to_string()), Ok(1_000_000_000_000));
    ...
    fn remove_missing_key_returns_false() {
        let mut table: ObjectLongHashtable<String> = ObjectLongHashtable::new();
    ...
    fn grows_beyond_initial_capacity_and_preserves_values() {
        let mut table = ObjectLongHashtable::with_capacity(2);
        for k in 0..50i64 {
    ...
    fn works_with_non_string_key_types() {
        let mut table: ObjectLongHashtable<(i32, i32)> = ObjectLongHashtable::new();
        table.put((1, 2), 12);
        table.put((3, 4), 34);
    ...
    fn default_matches_new() {
        let table: ObjectLongHashtable<String> = ObjectLongHashtable::default();
        assert_eq!(table.size(), 0);
    }
}
