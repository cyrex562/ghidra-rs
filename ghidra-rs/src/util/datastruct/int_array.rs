...
/// An array of `i32` that grows as needed.
///
/// Port of `ghidra.util.datastruct.IntArray`. An index that was never
...
impl IntArray {
    ...
    pub fn put(&mut self, index: usize, value: i32) {
        ...
        if index >= self.ints.len() {
            self.adjust_array(std::cmp::max(index as i64 + 1, self.ints.len() as i64 * 2));
        }
    ...
    pub fn remove(&mut self, index: usize) {
        ...
        if self.last_non_zero_index < self.ints.len() as i32 / 4 {
...
impl Array for IntArray {
    ...
    fn copy_data_to(&self, index: usize, table: &mut dyn DataTable, to_index: i32, to_col: i32) {
        table.put_int(to_index, to_col, self.get(index));
    }
}
...
mod tests {
    ...
    fn put_grows_backing_storage_past_min_size() {
        ...
        // Indexes in between remain zero.
    ...
    fn remove_shrinks_backing_storage() {
        ...
        arr.put(200, 1);
        arr.remove(200);
        // After removing the only element, the array should have shrunk back
        // down near MIN_SIZE rather than staying at >200 capacity.
        arr.put(3, 9);
        assert_eq!(arr.get(3), 9);
    }

    #[test]
    fn array_trait_copy_data_to_writes_int_column() {
        let mut arr = IntArray::new();
        arr.put(0, 99);
        let mut table = MockDataTable::default();
        Array::copy_data_to(&arr, 0, &mut table, 5, 1);
        assert_eq!(table.get_int(5, 1), 99);
    }
    ...
    fn default_matches_new() {
        assert_eq!(IntArray::default(), IntArray::new());
    }
}
