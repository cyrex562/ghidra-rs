...
/// An array of `i8` (Java `byte`) that grows as needed.
///
/// Port of `ghidra.util.datastruct.ByteArray`. An index that was never
...
impl ByteArray {
    ...
    pub fn put(&mut self, index: usize, value: i8) {
        if value == 0 {
            self.remove(index);
            return;
        }
        ...
        if index >= self.bytes.len() {
            self.adjust_array(std::cmp::max(index as i64 + 1, self.bytes.len() as i64 * 2));
        }
    ...
    pub fn remove(&mut self, index: usize) {
        ...
        if self.last_non_zero_index < self.bytes.len() as i32 / 4 {
...
impl Array for ByteArray {
    fn remove(&mut self, index: usize) {
        ByteArray::remove(self, index);
    }
...
mod tests {
    ...
    impl DataTable for MockDataTable {
        ...
        fn get_byte(&self, row: i32, col: i32) -> i8 {
            *self.bytes.get(&(row, col)).unwrap_or(&0)
        }
    ...
    #[test]
    fn array_trait_copy_data_to_writes_byte_column() {
        let mut arr = ByteArray::new();
        arr.put(0, 99);
        let mut table = MockDataTable::default();
        Array::copy_data_to(&arr, 0, &mut table, 5, 1);
        assert_eq!(table.get_byte(5, 1), 99);
    }
    ...
    fn default_matches_new() {
        assert_eq!(ByteArray::default(), ByteArray::new());
    }
}
