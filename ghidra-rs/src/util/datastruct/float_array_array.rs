...
/// An array of `Vec<f32>` (Java `float[]`) that grows as needed.
///
/// Port of `ghidra.util.datastruct.FloatArrayArray`. Backed by
...
pub struct FloatArrayArray {
    inner: PackedArrayArray<f32>,
}

impl FloatArrayArray {
    /// Creates a new, empty `FloatArrayArray`.
    pub fn new() -> Self {
    ...
    pub fn put(&mut self, index: usize, value: Option<&[f32]>) {
        self.inner.put(index, value);
    }

    /// Returns a copy of the `f32` slice stored at `index`, or `None` if not
...
impl Default for FloatArrayArray {
    fn default() -> Self {
        Self::new()
    }
}

impl Array for FloatArrayArray {
    fn remove(&mut self, index: usize) {
        FloatArrayArray::remove(self, index);
    }
    ...
    fn copy_data_to(&self, index: usize, table: &mut dyn DataTable, to_index: i32, to_col: i32) {
        table.put_float_array(to_index, to_col, self.get(index).unwrap_or_default());
    }
}
...
mod tests {
    ...
    impl DataTable for MockDataTable {
        ...
        fn put_float_array(&mut self, _row: i32, _col: i32, value: Vec<f32>) {
    ...
    fn put_and_get_roundtrip() {
        let mut arr = FloatArrayArray::new();
    ...
    #[test]
    fn array_trait_copy_data_to_writes_float_array_column() {
        let mut arr = FloatArrayArray::new();
        arr.put(0, Some(&[1.0, 2.0, 3.0]));
        let mut table = MockDataTable::default();
        Array::copy_data_to(&arr, 0, &mut table, 5, 1);
        assert_eq!(table.get_float_array(5, 1), vec![1.0, 2.0, 3.0]);
    }
}
