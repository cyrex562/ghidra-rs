//! Port of `ghidra.util.graph.attributes.IntegerAttribute`.

use std::cmp::Ordering;

use crate::util::datastruct::long_int_hashtable::LongIntHashtable;
use crate::util::exception::NoValueException;
use crate::util::graph::attributes::attribute::Attribute;
use crate::util::graph::attributes::attribute_manager::INTEGER_TYPE;
use crate::util::graph::key_indexable_set::KeyIndexableSet;
use crate::util::graph::keyed_object::KeyedObject;

/// Storage mechanism for integer-valued information about the elements of a `KeyIndexableSet`,
/// e.g. the vertices of a `DirectedGraph`.
///
/// Port of `ghidra.util.graph.attributes.IntegerAttribute<T extends KeyedObject>` (deprecated
/// since Ghidra 10.2). Values are stored in a [`LongIntHashtable`] keyed by [`KeyedObject::key`],
/// mirroring the Java field (`private LongIntHashtable values`; the commented-out `int[] values`
/// shown in the Java source was never actually used).
///
/// Unlike [`DoubleAttribute`](crate::util::graph::attributes::DoubleAttribute), whose backing
/// `LongDoubleHashtable` is constructed with no size hint (`new LongDoubleHashtable()`), Java's
/// `IntegerAttribute` constructor passes the owning set's current size as an initial capacity
/// hint (`new ghidra.util.datastruct.LongIntHashtable(set.size())`) -- mirrored here via
/// [`LongIntHashtable::with_capacity`] rather than [`LongIntHashtable::new`].
///
...
pub struct IntegerAttribute<'a, T: KeyedObject> {
    name: String,
    owning_set: &'a dyn KeyIndexableSet<T>,
    modification_number: i64,
    backing_set_modification_number: i64,
    values: LongIntHashtable,
}

#[allow(deprecated)]
impl<'a, T: KeyedObject> IntegerAttribute<'a, T> {
    /// Construct a new `IntegerAttribute`.
    ///
    /// Port of `IntegerAttribute(String name, KeyIndexableSet<T> set)`.
    ///
    ...
    pub fn new(name: impl Into<String>, set: &'a dyn KeyIndexableSet<T>) -> Self {
        Self {
            ...
            values: LongIntHashtable::with_capacity(set.size() as i32),
        }
    }

    /// Set the value of this attribute for the specified `KeyedObject`.
    ///
    /// Port of `setValue(T o, int value)`. Unlike
    /// [`DoubleAttribute::set_value`](crate::util::graph::attributes::DoubleAttribute::set_value),
    /// Java's `IntegerAttribute.setValue` does not check `owningSet().contains(o)` before storing
    /// -- it always calls `update()` and stores the value, mirrored here exactly.
    ///
    ...
    pub fn set_value(&mut self, o: &T, value: i32) {
        self.update();
    ...
    pub fn get_value(&self, o: &dyn KeyedObject) -> Result<i32, NoValueException> {
    ...
    /// docs for why this returns a freshly-sorted `Vec` (mirroring Java sorting a *clone* of the
    /// input array) rather than sorting in place, and why it tolerates `KeyedObject`s outside the
    /// owning set.
    ...
    /// Port of the nested `IntegerAttribute.IntegerComparator.compare(Object, Object)`.
    ///
    ...
    /// key. This is the exact same nested-try/catch shape as
    /// [`DoubleAttribute`](crate::util::graph::attributes::DoubleAttribute)'s own
    /// `DoubleComparator` (see that struct's `compare` docs for the full trace), just over `i32`
    /// values compared via subtraction (`value1 - value2`) instead of `f64` subtraction.
...
#[allow(deprecated)]
impl<'a, T: KeyedObject> Attribute<T> for IntegerAttribute<'a, T> {
    fn name(&self) -> &str {
        &self.name
    }
    ...
    /// Port of `attributeType()`: returns `AttributeManager.INTEGER_TYPE`.
    fn attribute_type(&self) -> String {
        INTEGER_TYPE.to_string()
    }

    /// Port of `getValueAsString(KeyedObject o)`: `Integer.toString(getValue(o))`, or `"0"` if
...
mod tests {
    ...
    fn name_and_type_report_constructed_values() {
        let set = set_with(&[]);
        let attr = IntegerAttribute::new("count", &set);
        assert_eq!(attr.name(), "count");
        assert_eq!(attr.attribute_type(), "INTEGER_TYPE");
    }

    #[test]
    fn set_value_then_get_value_round_trips() {
        let set = set_with(&[1, 2]);
        let mut attr = IntegerAttribute::new("count", &set);
        let obj = MockKeyedObject { key: 1 };
        attr.set_value(&obj, 42);
        assert_eq!(attr.get_value(&obj), Ok(42));
    }

    #[test]
    fn set_value_does_not_check_owning_set_membership() {
        // Preserved quirk: unlike DoubleAttribute::set_value, IntegerAttribute::set_value stores
        // (and bumps the modification number) unconditionally, even for a stray KeyedObject.
        let set = set_with(&[1]);
        let mut attr = IntegerAttribute::new("count", &set);
    ...
    fn get_value_for_unset_object_is_no_value_exception() {
        let set = set_with(&[1]);
        let attr: IntegerAttribute<MockKeyedObject> = IntegerAttribute::new("count", &set);
    ...
    fn boxed_trait_object_is_usable() {
        let set = set_with(&[1]);
        let mut attr: Box<dyn Attribute<MockKeyedObject>> = Box::new(IntegerAttribute::new("count", &set));
        assert_eq!(attr.name(), "count");
        assert_eq!(attr.attribute_type(), "INTEGER_TYPE");
        attr.clear();
    }
