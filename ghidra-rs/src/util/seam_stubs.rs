...
// `ghidra.util.bytesearch.Pattern` is now the real port at
// `crate::util::bytesearch::pattern::Pattern` (a concrete struct, per its shape rule); the
// placeholder that used to live here (along with its `FileMarker`/`SAXParseException` satellite
// placeholders, orphaned once the real `Pattern` was scoped down -- see that struct's module
// docs for what was and wasn't ported) has been removed.
...
/// Placeholder for `ghidra.util.filechooser.ResourceFile`, referenced by
/// [`bundle_status_change_request_listener`](crate::app::plugin::core::osgi::bundle_status_change_request_listener).
