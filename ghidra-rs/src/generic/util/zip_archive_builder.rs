//! Port of `generic.util.ZipArchiveBuilder`.
//!
//! Builds a zip archive by writing entries to a file on disk.
...
use super::archive_builder::ArchiveBuilder;

/// Builds a zip archive to a file on disk.
///
/// This is the direct analogue of `generic.util.ZipArchiveBuilder`, which wraps a
/// `java.util.zip.ZipOutputStream` and writes to a file.
///
...
impl ZipArchiveBuilder {
    /// Creates a new zip archive builder, opening a file for writing at the given path.
    ///
    /// Mirrors the Java `ZipArchiveBuilder(File outputFile)` constructor.
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    fn new_creates_file() {
        ...
        let builder = ZipArchiveBuilder::new(&temp_path).unwrap();
    ...
    fn add_file_creates_archive_entry() {
        ...
        let mut builder = ZipArchiveBuilder::new(&temp_path).unwrap();
