...
impl MessageLog {
    ...
    pub fn copy_from(&mut self, log: &MessageLog) {
        for other_message in log.messages.clone() {
    ...
    pub fn append_msg_from(&mut self, originator: Option<&str>, message: &str) {
        match originator {
    ...
    pub fn append_msg_at_line(&mut self, line_num: i32, message: &str) {
    ...
    /// is supplied explicitly by the caller rather than captured internally.
    pub fn append_exception(&mut self, error: &dyn std::error::Error, trace: &[StackFrame]) {
    ...
    pub fn error(&mut self, originator: Option<&str>, message: &str) {
...
mod tests {
    ...
    fn copy_from_appends_other_logs_messages() {
        let mut source = MessageLog::new();
        source.append_msg("first");
        source.append_msg("second");

        let mut dest = MessageLog::new();
