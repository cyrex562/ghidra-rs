...
/// A command to set a fallthrough override on an instruction.
pub struct SetFallThroughCmd {
    inst_addr: Address,
    fallthrough_addr: Address,
}

impl SetFallThroughCmd {
    /// Creates a new command to set a fallthrough address on an instruction.
    ///
    /// # Arguments
    ///
    /// * `inst_addr` - the address of the instruction whose fallthrough property is to be set.
    /// * `fallthrough_addr` - the address to use for the instruction's fallthrough.
    pub fn new(inst_addr: Address, fallthrough_addr: Address) -> Self {
        SetFallThroughCmd {
            inst_addr,
            fallthrough_addr,
        }
...
impl Command<dyn Program + 'static> for SetFallThroughCmd {
    fn apply_to(&mut self, program: &mut (dyn Program + 'static)) -> bool {
        if let Some(listing) = program.get_listing() {
            if let Some(inst) = listing.get_instruction_at(&self.inst_addr) {
                ...
                unsafe {
                    (*inst_ptr).set_fall_through(Some(self.fallthrough_addr.clone()));
                }
                return true;
            }
        }
    ...
    fn name(&self) -> String {
        "Set Fall-Through Address".to_string()
    }
}
...
mod tests {
    ...
    impl Instruction for MockInstruction {
        ...
        fn set_fall_through(&mut self, addr: Option<Address>) {
            self.fall_through_addr = addr;
        }

        fn is_fall_through_overridden(&self) -> bool {
            self.fall_through_addr.is_some()
        }
    ...
    fn apply_to_sets_fall_through() {
        ...
        let mut cmd = SetFallThroughCmd::new(inst_addr, fallthrough_addr.clone());
        ...
        assert_eq!(mock_inst.fall_through_addr, Some(fallthrough_addr));
    }
