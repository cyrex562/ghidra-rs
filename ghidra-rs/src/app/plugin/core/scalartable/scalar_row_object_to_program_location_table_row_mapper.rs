//! Port of `ghidra.app.plugin.core.scalartable.ScalarRowObjectToProgramLocationTableRowMapper`.
...
///
/// Same approach as
/// [`AddressableRowObjectToProgramLocationTableRowMapper`](crate::app::tablechooser::addressable_row_object_to_program_location_table_row_mapper::AddressableRowObjectToProgramLocationTableRowMapper)'s
...
/// Maps a [`ScalarRowObject`] to a [`ProgramLocation`] over its [`Address`], letting columns
/// designed for program-location tables be reused by the Scalar table.
///
/// Port of `ghidra.app.plugin.core.scalartable.ScalarRowObjectToProgramLocationTableRowMapper`,
/// which `extends ProgramLocationTableRowMapper<ScalarRowObject, ProgramLocation>`. As with this
...
pub struct ScalarRowObjectToProgramLocationTableRowMapper;

impl TableRowMapper<ScalarRowObject, Box<dyn ProgramLocation>> for ScalarRowObjectToProgramLocationTableRowMapper {
    fn map(
        &self,
        ...
        Box::new(MappedProgramLocation { program, address: row_object.get_address() })
    }
}

impl ProgramLocationTableRowMapper<ScalarRowObject, Box<dyn ProgramLocation>>
    for ScalarRowObjectToProgramLocationTableRowMapper
{
}
...
mod tests {
    ...
    fn map_returns_a_program_location_over_the_row_objects_address() {
        let mapper = ScalarRowObjectToProgramLocationTableRowMapper;
        let row = scalar_row_object(0x5000);
    ...
    fn mapped_locations_program_carries_the_source_programs_identity() {
        let mapper = ScalarRowObjectToProgramLocationTableRowMapper;
        let row = scalar_row_object(0x10);
        let program = MockProgram;
        let provider = MockServiceProvider;
    ...
    fn different_row_objects_map_to_their_own_locations() {
        ...
        let a = scalar_row_object(0x100);
        let b = scalar_row_object(0x200);
