...
const NAME: &str = "Trace Opened @";

/// Event fired when a trace is opened.
///
/// Mirrors `ghidra.app.plugin.core.debug.event.TraceOpenedPluginEvent`.
pub struct TraceOpenedPluginEvent {
    event: PluginEvent,
    new_trace_ref: Weak<dyn Trace>,
}

impl TraceOpenedPluginEvent {
    /// Creates a new trace opened event.
    ///
    ...
    pub fn new(source: impl Into<String>, trace: Arc<dyn Trace>) -> Self {
        ...
        let new_trace_ref = Arc::downgrade(&trace);
...
mod tests {
    ...
    fn new_stores_source_and_trace() {
        ...
        let event = TraceOpenedPluginEvent::new("TestSource", trace.clone());
    ...
    fn event_name_includes_trace_pointer() {
        ...
        let expected = format!("Trace Opened @{:x}", trace_ptr);
    ...
    fn event_name_constant_is_correct() {
        assert_eq!(NAME, "Trace Opened @");
    }

    #[test]
    fn event_mut_allows_modification() {
        let trace = Arc::new(MockTrace);
        let mut event = TraceOpenedPluginEvent::new("TestSource", trace);
