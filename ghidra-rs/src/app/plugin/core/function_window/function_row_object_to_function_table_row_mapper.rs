//! Port of `ghidra.app.plugin.core.functionwindow.FunctionRowObjectToFunctionTableRowMapper`.
...
/// Maps a [`FunctionRowObject`] to its wrapped [`Function`], letting columns designed for
/// function tables be reused directly.
///
/// Port of `ghidra.app.plugin.core.functionwindow.FunctionRowObjectToFunctionTableRowMapper`,
/// which `extends ProgramLocationTableRowMapper<FunctionRowObject, Function>`. As with this
/// crate's other row-mapper ports (e.g.
/// [`FunctionRowObjectToAddressTableRowMapper`](crate::app::plugin::core::function_window::function_row_object_to_address_table_row_mapper::FunctionRowObjectToAddressTableRowMapper)),
...
pub struct FunctionRowObjectToFunctionTableRowMapper;

impl TableRowMapper<FunctionRowObject, Arc<dyn Function>> for FunctionRowObjectToFunctionTableRowMapper {
    fn map(
        &self,
        ...
        Arc::clone(row_object.get_function())
    }
}

impl ProgramLocationTableRowMapper<FunctionRowObject, Arc<dyn Function>>
    for FunctionRowObjectToFunctionTableRowMapper
{
}
...
mod tests {
    ...
    fn function_row_object(entry_point: Address) -> (FunctionRowObject, Arc<dyn Function>) {
        let function: Arc<dyn Function> = Arc::new(MockFunction { entry_point });
        (FunctionRowObject::new(Arc::clone(&function)), function)
    }

    #[test]
    fn map_returns_the_wrapped_function() {
        let mapper = FunctionRowObjectToFunctionTableRowMapper;
        let (row, function) = function_row_object(ram_address(0x400000));
        let program = MockProgram;
        let provider = MockServiceProvider;

        let mapped = mapper.map(&row, &program, &provider);

        assert!(Arc::ptr_eq(&mapped, &function));
        assert_eq!(mapped.get_entry_point(), ram_address(0x400000));
    }

    #[test]
    fn different_row_objects_map_to_their_own_functions() {
        let mapper = FunctionRowObjectToFunctionTableRowMapper;
        let program = MockProgram;
        let provider = MockServiceProvider;

        let (a, fn_a) = function_row_object(ram_address(0x1000));
        let (b, fn_b) = function_row_object(ram_address(0x2000));

        assert!(Arc::ptr_eq(&mapper.map(&a, &program, &provider), &fn_a));
        assert!(Arc::ptr_eq(&mapper.map(&b, &program, &provider), &fn_b));
    }
}
