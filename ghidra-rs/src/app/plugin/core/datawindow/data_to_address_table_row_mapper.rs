//! Port of `ghidra.app.plugin.core.datawindow.DataToAddressTableRowMapper`.
...
/// Maps a [`Data`] to its minimum [`Address`], letting columns designed for address tables be
/// reused by the Data table.
///
/// Port of `ghidra.app.plugin.core.datawindow.DataToAddressTableRowMapper`, which `extends
/// ProgramLocationTableRowMapper<Data, Address>`. As with this crate's other row-mapper ports
/// (e.g.
/// [`ScalarRowObjectToAddressTableRowMapper`](crate::app::plugin::core::scalartable::scalar_row_object_to_address_table_row_mapper::ScalarRowObjectToAddressTableRowMapper)),
...
/// (an interface), not a dedicated row-object wrapper class -- ported as `Box<dyn Data>`.
///
...
impl TableRowMapper<Box<dyn Data>, Address> for DataToAddressTableRowMapper {
    fn map(
        &self,
        ...
        row_object.get_min_address()
    }
}

impl ProgramLocationTableRowMapper<Box<dyn Data>, Address> for DataToAddressTableRowMapper {}
...
mod tests {
    ...
    #[test]
    fn map_returns_the_datas_min_address() {
        let mapper = DataToAddressTableRowMapper;
        let row = data_at(0x4000);
    ...
    #[test]
    fn different_data_rows_map_to_their_own_addresses() {
        let mapper = DataToAddressTableRowMapper;
        let program = MockProgram;
        let provider = MockServiceProvider;
