//! Mirrors `ghidra.app.plugin.languages.sleigh.SubtableEntryVisitor`.
...
///
/// Mirrors `ghidra.app.plugin.languages.sleigh.SubtableEntryVisitor`, which extends
...
pub trait SubtableEntryVisitor {
    ...
    fn visit(&mut self, pattern: &DisjointPattern, cons: &Constructor) -> VisitorResult;
}
...
mod tests {
    ...
    /// Counts constructors visited, terminating early once a target count is reached --
    ...
    impl SubtableEntryVisitor for CountingVisitor {
    ...
    fn object_safe_mock_visits_and_terminates() {
        ...
        let boxed: &mut dyn SubtableEntryVisitor = &mut visitor;

        let pattern = dummy_pattern();
        let cons = dummy_constructor();

        assert_eq!(boxed.visit(&pattern, &cons), VisitorResult::Continue);
        assert_eq!(boxed.visit(&pattern, &cons), VisitorResult::Finished);
