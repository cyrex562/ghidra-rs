...
const NAME: &str = "Program Opened";

/// Event for notification of a program being opened.
///
/// Mirrors `ghidra.app.events.ProgramOpenedPluginEvent`.
...
impl ProgramOpenedPluginEvent {
    /// Creates a new program opened event.
    ///
    /// # Arguments
    ///
    ...
    /// Returns the program that has just been opened, or `None` if it has been dropped.
    ///
    /// In the original Java, this can return null, but only if the program has been closed and
    /// is no longer in use, which can't happen if the method is called during the original event
    /// notification.
    ///
    /// Mirrors `getProgram()`.
...
mod tests {
    ...
    fn new_stores_source() {
        ...
        let event = ProgramOpenedPluginEvent::new("TestPlugin", program);
    ...
    fn new_stores_program() {
        let program = std::sync::Arc::new(MockProgram);
        let event = ProgramOpenedPluginEvent::new("TestPlugin", program.clone());
    ...
    fn event_name_is_correct() {
        ...
        assert_eq!(event.event().event_name(), "Program Opened");
    }
    ...
    fn event_mut_allows_modification() {
        let program = std::sync::Arc::new(MockProgram);
        let mut event = ProgramOpenedPluginEvent::new("TestPlugin", program);
