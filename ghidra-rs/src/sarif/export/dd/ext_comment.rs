...
use crate::sarif::managers::CommentsSarifMgr;

/// Represents a comment on a [`Data`] object for SARIF export.
///
/// Port of `sarif.export.dd.ExtComment`.
pub struct ExtComment {
    pub comment_type: String,
    pub comment: Option<String>,
}

impl ExtComment {
    /// Creates a new `ExtComment` describing `data`'s comment of the given `comment_type`.
    ///
    /// Java: `ExtComment(Data data, CommentType type)`.
    pub fn new(data: &dyn Data, comment_type: CommentType) -> Self {
        Self {
            comment_type: CommentsSarifMgr::get_comment_type_string(comment_type).to_string(),
            comment: data.get_comment(comment_type),
        }
    }
}

impl IsfObject for ExtComment {}
...
mod tests {
    ...
    /// Minimal [`Data`] mock reporting a fixed comment for one [`CommentType`] and `None`
    /// otherwise, enough to exercise [`ExtComment::new`] without pulling in a real database.
    struct MockData {
        stored_type: CommentType,
        stored_comment: Option<String>,
    }
    ...
    impl CodeUnit for MockData {
        ...
        fn get_comment(&self, comment_type: CommentType) -> Option<String> {
            if comment_type == self.stored_type {
                self.stored_comment.clone()
            } else {
                None
            }
        }
    ...
    impl Data for MockData {
        ...
        fn get_root(&self) -> Box<dyn Data> {
            Box::new(MockData { stored_type: self.stored_type, stored_comment: self.stored_comment.clone() })
        }
    ...
    #[test]
    fn captures_comment_type_string_and_present_comment() {
        let data = MockData { stored_type: CommentType::Eol, stored_comment: Some("hello".to_string()) };
        let ext = ExtComment::new(&data, CommentType::Eol);
        assert_eq!(ext.comment_type, CommentsSarifMgr::get_comment_type_string(CommentType::Eol));
        assert_eq!(ext.comment.as_deref(), Some("hello"));
    }

    #[test]
    fn missing_comment_is_none() {
        let data = MockData { stored_type: CommentType::Eol, stored_comment: None };
        let ext = ExtComment::new(&data, CommentType::Eol);
        assert_eq!(ext.comment, None);
    }

    #[test]
    fn distinguishes_comment_type_from_stored_comment_type() {
        let data = MockData { stored_type: CommentType::Eol, stored_comment: Some("hi".to_string()) };
        // Requesting a different comment type than the one MockData actually has set: the
        // comment_type field still reports the requested type, but the comment is None.
        let ext = ExtComment::new(&data, CommentType::Plate);
        assert_eq!(ext.comment_type, CommentsSarifMgr::get_comment_type_string(CommentType::Plate));
        assert_eq!(ext.comment, None);
    }
