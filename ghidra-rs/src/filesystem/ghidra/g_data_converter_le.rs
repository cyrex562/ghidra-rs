//! Little-endian implementation of [`GDataConverter`].
//!
//! Mirrors `mobiledevices.dmg.ghidra.GDataConverterLE` from the original Ghidra source.

use super::g_data_converter::GDataConverter;

/// Converts between byte slices and primitive integer types using little-endian byte order.
///
/// A shared singleton is available via [`INSTANCE`].
pub struct GDataConverterLe;

/// Shared singleton, mirroring `GDataConverterLE.INSTANCE` in the original Java source.
pub static INSTANCE: GDataConverterLe = GDataConverterLe;

impl GDataConverter for GDataConverterLe {
...
mod tests {
    ...
    fn get_int_sign_extends_msb() {
        assert_eq!(INSTANCE.get_int(&[0x00, 0x00, 0x00, 0x80]), i32::MIN);
    }
    ...
    fn get_long_sign_extends_msb() {
        assert_eq!(
            INSTANCE.get_long(&[0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80]),
            i64::MIN
        );
    }
    ...
    fn get_value_two_bytes() {
        assert_eq!(INSTANCE.get_value(&[0x02, 0x01], 2), 0x0102);
    }
    ...
    fn get_value_at_offset() {
        let buf = [0x00, 0x00, 0x03, 0x02, 0x01];
    ...
    fn put_long_sized_partial() {
        let mut b = [0u8; 4];
        // Write the 3 LSBs of 0x00010203 in little-endian at offset 0
        INSTANCE.put_long_sized(&mut b, 0, 0x00010203, 3);
    ...
    #[test]
    fn short_to_bytes() {
        assert_eq!(INSTANCE.short_to_bytes(0x0102), [0x02, 0x01]);
    }

    #[test]
    fn int_to_bytes() {
        assert_eq!(INSTANCE.int_to_bytes(0x01020304), [0x04, 0x03, 0x02, 0x01]);
    }
