...
///
/// Corresponds to `ghidra.pcodeCPort.address.AddressUtils`.
...
mod tests {
    ...
    fn compare_both_negative_as_signed() {
        // Both have high bit set; larger unsigned value wins
        assert_eq!(unsigned_compare(-2, -1), -1);
        assert_eq!(unsigned_compare(-1, -2), 1);
    }
    ...
    fn subtract_wraps_on_underflow() {
        // 0u64 - 1 wraps to u64::MAX, reinterpreted as -1i64
        assert_eq!(unsigned_subtract(0, 1), -1i64);
