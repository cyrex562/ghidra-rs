...
/// A bitwise NOT operator for pattern expressions.
///
/// NotExpression represents the bitwise negation (NOT) of a pattern expression.
/// When encoded, it wraps the operand's encoding in a NOT_EXP element tag.
///
/// Models `ghidra.pcodeCPort.slghpatexpress.NotExpression`.
...
impl NotExpression {
    /// Creates a new not expression with the given location.
    ///
    ...
    /// Creates a new not expression with a sub-expression operand.
    ///
    ...
    /// Returns the sub-expression of this not operation.
    pub fn get_unary(&self) -> Option<&dyn PatternExpression> {
        self.unary.get_unary()
    }

    /// Encodes this not expression to the given encoder.
    ///
    /// Wraps the operand's encoding in ELEM_NOT_EXP tags.
    pub fn encode(&self, encoder: &mut dyn Encoder) -> io::Result<()> {
        encoder.open_element(ELEM_NOT_EXP)?;
        if let Some(operand) = self.get_unary() {
            operand.encode(encoder)?;
        }
        encoder.close_element(ELEM_NOT_EXP)?;
        Ok(())
    }
}
...
mod tests {
    ...
    fn new_creates_with_location() {
        ...
        let _expr = NotExpression::new(location);
    }
    ...
    fn get_unary_returns_none_when_empty() {
        let location = Location::new("test.sleigh", 1);
        let expr = NotExpression::new(location);
