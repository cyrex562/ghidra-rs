//! Models `ghidra.pcodeCPort.slghpatexpress.NotEqualEquation`.
...
/// An equation that constrains a pattern value to differ from one of the values its
/// right-hand pattern expression can take on.
///
/// Models `ghidra.pcodeCPort.slghpatexpress.NotEqualEquation`; see
/// [`GreaterEqualEquation`](super::greater_equal_equation::GreaterEqualEquation) for why this
/// is exposed as a trait.
pub trait NotEqualEquation: Send + Sync {
    ...
    /// Returns the token pattern computed by the most recent [`NotEqualEquation::gen_pattern`]
    /// call, if any.
    fn get_token_pattern(&self) -> Option<&dyn TokenPattern>;
}

/// The concrete `ghidra.pcodeCPort.slghpatexpress.NotEqualEquation`.
pub struct NotEqualEquationImpl {
    equation: ValExpressEquation,
    token_pattern: Option<Box<dyn TokenPattern>>,
}

impl NotEqualEquationImpl {
    /// Creates a new not-equal-constraint equation from its left- and right-hand operands.
    ...
    /// The pattern expression `lhs` must differ from at least one value of (the Java `rhs`
    /// field).
...
impl NotEqualEquation for NotEqualEquationImpl {
    fn gen_pattern(&mut self) {
        let pattern = gen_comparison_pattern(
            ...
            "Notequal constraint is impossible to match",
        );
...
mod tests {
    ...
    #[test]
    fn gen_pattern_matches_when_some_lhsval_differs_from_a_value() {
        ...
        let mut equation = NotEqualEquationImpl::new(loc(), lhs, rhs);
    ...
    #[test]
    #[should_panic(expected = "Notequal constraint is impossible to match")]
    fn gen_pattern_panics_when_lhs_is_a_single_value_equal_to_the_only_rhs_value() {
    ...
    fn get_lhs_and_rhs_expose_operands() {
        ...
        let equation = NotEqualEquationImpl::new(loc(), lhs, rhs);
