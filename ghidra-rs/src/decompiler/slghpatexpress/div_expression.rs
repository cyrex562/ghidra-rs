//! Models `ghidra.pcodeCPort.slghpatexpress.DivExpression`.
...
use crate::program::model::pcode::ids::ELEM_DIV_EXP;
use crate::sleigh::grammar::Location;
use std::io;

/// A division operator for pattern expressions.
///
/// DivExpression represents the integer division of two pattern expressions.
/// When encoded, it wraps the operands' encoding in a DIV_EXP element tag.
///
/// Models `ghidra.pcodeCPort.slghpatexpress.DivExpression`.
...
impl DivExpression {
    /// Creates a new div expression with the given location.
    ///
    ...
    /// Creates a new div expression with left and right sub-expression operands.
    ///
    ...
    /// Returns the left sub-expression of this div operation.
    pub fn get_left(&self) -> Option<&dyn PatternExpression> {
        self.binary.get_left()
    }

    /// Returns the right sub-expression of this div operation.
    pub fn get_right(&self) -> Option<&dyn PatternExpression> {
        self.binary.get_right()
    }

    /// Encodes this div expression to the given encoder.
    ///
    /// Wraps the operands' encoding in ELEM_DIV_EXP tags.
    pub fn encode(&self, encoder: &mut dyn Encoder) -> io::Result<()> {
        encoder.open_element(ELEM_DIV_EXP)?;
        self.binary.encode(encoder)?;
        encoder.close_element(ELEM_DIV_EXP)?;
        Ok(())
    }
}

impl PatternExpression for DivExpression {
    ...
    /// Models `DivExpression.getSubValue`: evaluates the left operand first (Java relies on this
    ...
    fn get_sub_value(&self, replace: &VectorStl<i64>, listpos: &mut MutableInt) -> i64 {
        let left = self.binary.get_left().expect("DivExpression requires a left operand");
        let leftval = left.get_sub_value(replace, listpos); // Must be left first
        let right = self.binary.get_right().expect("DivExpression requires a right operand");
...
mod tests {
    ...
    fn new_creates_with_location() {
        ...
        let _expr = DivExpression::new(location);
    }
    ...
    fn get_left_and_right_return_none_when_empty() {
        let location = Location::new("test.sleigh", 1);
        let expr = DivExpression::new(location);
