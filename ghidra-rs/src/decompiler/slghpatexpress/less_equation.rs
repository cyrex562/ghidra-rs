//! Models `ghidra.pcodeCPort.slghpatexpress.LessEquation`.
...
/// An equation that constrains a pattern value to be strictly less than one of the values its
/// right-hand pattern expression can take on.
///
/// Models `ghidra.pcodeCPort.slghpatexpress.LessEquation`; see
/// [`GreaterEqualEquation`](super::greater_equal_equation::GreaterEqualEquation) for why this
/// is exposed as a trait.
pub trait LessEquation: Send + Sync {
    ...
    /// Mirrors Java's own message, which reads "LessEquation constraint is impossible to
    /// match" rather than "Less than constraint..." -- a copy-paste artifact from
    /// `GreaterEquation`, preserved here for fidelity.
    fn gen_pattern(&mut self);

    /// Returns the token pattern computed by the most recent [`LessEquation::gen_pattern`]
    /// call, if any.
    fn get_token_pattern(&self) -> Option<&dyn TokenPattern>;
}

/// The concrete `ghidra.pcodeCPort.slghpatexpress.LessEquation`.
pub struct LessEquationImpl {
    equation: ValExpressEquation,
    token_pattern: Option<Box<dyn TokenPattern>>,
}

impl LessEquationImpl {
    /// Creates a new less-than-constraint equation from its left- and right-hand operands.
...
impl LessEquation for LessEquationImpl {
    fn gen_pattern(&mut self) {
        let pattern = gen_comparison_pattern(
            ...
            "LessEquation constraint is impossible to match",
        );
...
mod tests {
    ...
    #[test]
    fn gen_pattern_matches_when_some_lhsval_is_lt_a_value() {
        ...
        let mut equation = LessEquationImpl::new(loc(), lhs, rhs);
    ...
    #[test]
    #[should_panic(expected = "LessEquation constraint is impossible to match")]
    fn gen_pattern_panics_when_no_lhsval_is_below_any_value() {
        let lhs: Box<dyn PatternValue> = Box::new(LeafValue::new(loc(), 100, 104, 0));
        let rhs: Box<dyn PatternExpression> = Box::new(FixedValuesExpression::new(vec![5]));
        let mut equation = LessEquationImpl::new(loc(), lhs, rhs);
    ...
    fn get_lhs_and_rhs_expose_operands() {
        ...
        let equation = LessEquationImpl::new(loc(), lhs, rhs);
