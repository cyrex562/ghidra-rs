//! Models `ghidra.pcodeCPort.slghpatexpress.EquationOr`.

use crate::decompiler::slghpatexpress::{OperandResolve, PatternEquationOps, TokenPattern};
use crate::sleigh::grammar::Location;

/// Combines two pattern equations with an OR operation.
///
/// An equation that represents the logical OR of two pattern equations.
/// Either sub-equation can be satisfied for this equation to match.
///
/// Models `ghidra.pcodeCPort.slghpatexpress.EquationOr`.
pub struct EquationOr {
    location: Location,
...
impl EquationOr {
    /// Creates a new equation that ORs two pattern equations together.
    ///
    /// # Arguments
    ///
    ...
    /// Generates the token pattern for this equation by ORing the patterns of both operands.
    ///
    /// Models Java's `genPattern()` method behavior:
    /// - Calls genPattern on both left and right operands
    /// - Combines their patterns using doOr()
    pub fn gen_pattern(&mut self) {
        ...
        {
            let combined = left_pattern.do_or(right_pattern);
...
mod tests {
    ...
    fn new_stores_location_and_operands() {
        ...
        let equation = EquationOr::new(location.clone(), left, right);
    ...
    fn gen_pattern_combines_operand_patterns() {
        ...
        let mut equation = EquationOr::new(
            location.clone(),
            Box::new(left),
            Box::new(right),
        );
    ...
    fn resolve_operand_left_delegates_to_operands() {
        ...
        let left = Box::new(MockEquation::new());
        let right = Box::new(MockEquation::new());
        let equation = EquationOr::new(location, left, right);
