//! Models `ghidra.pcodeCPort.slghpatexpress.MultExpression`.
...
use crate::program::model::pcode::ids::ELEM_MULT_EXP;
use crate::sleigh::grammar::Location;
use std::io;

/// A multiplication operator for pattern expressions.
///
/// MultExpression represents the product of two pattern expressions.
/// When encoded, it wraps the operands' encoding in a MULT_EXP element tag.
///
/// Models `ghidra.pcodeCPort.slghpatexpress.MultExpression`.
...
impl MultExpression {
    /// Creates a new mult expression with the given location.
    ///
    ...
    /// Creates a new mult expression with left and right sub-expression operands.
    ///
    ...
    /// Returns the left sub-expression of this mult operation.
    pub fn get_left(&self) -> Option<&dyn PatternExpression> {
        self.binary.get_left()
    }

    /// Returns the right sub-expression of this mult operation.
    pub fn get_right(&self) -> Option<&dyn PatternExpression> {
        self.binary.get_right()
    }

    /// Encodes this mult expression to the given encoder.
    ///
    /// Wraps the operands' encoding in ELEM_MULT_EXP tags.
    pub fn encode(&self, encoder: &mut dyn Encoder) -> io::Result<()> {
        encoder.open_element(ELEM_MULT_EXP)?;
        self.binary.encode(encoder)?;
        encoder.close_element(ELEM_MULT_EXP)?;
        Ok(())
    }
}

impl PatternExpression for MultExpression {
    ...
    /// Models `MultExpression.getSubValue`: evaluates the left operand first (Java relies on this
    ...
    fn get_sub_value(&self, replace: &VectorStl<i64>, listpos: &mut MutableInt) -> i64 {
        let left = self.binary.get_left().expect("MultExpression requires a left operand");
        let leftval = left.get_sub_value(replace, listpos); // Must be left first
        let right = self.binary.get_right().expect("MultExpression requires a right operand");
...
mod tests {
    ...
    fn new_creates_with_location() {
        ...
        let _expr = MultExpression::new(location);
    }
    ...
    fn with_operands_stores_both_operands() {
        ...
        let expr = MultExpression::with_operands(location, left, right);
    ...
    fn get_left_and_right_return_none_when_empty() {
        let location = Location::new("test.sleigh", 1);
        let expr = MultExpression::new(location);
