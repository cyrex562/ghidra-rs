...
impl PatternExpression for OrExpression {
    ...
    /// Models `OrExpression.getSubValue`: evaluates the left operand first (Java relies on this
    ...
    fn get_sub_value(&self, replace: &VectorStl<i64>, listpos: &mut MutableInt) -> i64 {
        let left = self.binary.get_left().expect("OrExpression requires a left operand");
        let leftval = left.get_sub_value(replace, listpos); // Must be left first
        let right = self.binary.get_right().expect("OrExpression requires a right operand");
