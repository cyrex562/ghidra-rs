//! Models `ghidra.pcodeCPort.slghpatexpress.LeftShiftExpression`.
...
use crate::program::model::pcode::ids::ELEM_LSHIFT_EXP;
use crate::sleigh::grammar::Location;
use std::io;

/// A bitwise left shift operator for pattern expressions.
///
/// LeftShiftExpression represents the left shift of two pattern expressions.
/// When encoded, it wraps the operands' encoding in a LSHIFT_EXP element tag.
///
/// Models `ghidra.pcodeCPort.slghpatexpress.LeftShiftExpression`.
...
impl LeftShiftExpression {
    /// Creates a new left shift expression with the given location.
    ///
    ...
    /// Creates a new left shift expression with left and right sub-expression operands.
    ///
    ...
    /// Returns the left sub-expression of this left shift operation.
    pub fn get_left(&self) -> Option<&dyn PatternExpression> {
        self.binary.get_left()
    }

    /// Returns the right sub-expression of this left shift operation.
    pub fn get_right(&self) -> Option<&dyn PatternExpression> {
        self.binary.get_right()
    }

    /// Encodes this left shift expression to the given encoder.
    ///
    /// Wraps the operands' encoding in ELEM_LSHIFT_EXP tags.
    pub fn encode(&self, encoder: &mut dyn Encoder) -> io::Result<()> {
        encoder.open_element(ELEM_LSHIFT_EXP)?;
        self.binary.encode(encoder)?;
        encoder.close_element(ELEM_LSHIFT_EXP)?;
        Ok(())
    }
}

impl PatternExpression for LeftShiftExpression {
    ...
    /// Models `LeftShiftExpression.getSubValue`: evaluates the left operand first (Java relies on this
    ...
    fn get_sub_value(&self, replace: &VectorStl<i64>, listpos: &mut MutableInt) -> i64 {
        let left = self.binary.get_left().expect("LeftShiftExpression requires a left operand");
        let leftval = left.get_sub_value(replace, listpos); // Must be left first
        let right = self.binary.get_right().expect("LeftShiftExpression requires a right operand");
...
mod tests {
    ...
    fn new_creates_with_location() {
        ...
        let _expr = LeftShiftExpression::new(location);
    }
    ...
    fn with_operands_stores_both_operands() {
        ...
        let expr = LeftShiftExpression::with_operands(location, left, right);
    ...
    fn get_left_and_right_return_none_when_empty() {
        let location = Location::new("test.sleigh", 1);
        let expr = LeftShiftExpression::new(location);
