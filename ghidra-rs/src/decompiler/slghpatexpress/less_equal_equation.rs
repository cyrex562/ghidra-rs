//! Models `ghidra.pcodeCPort.slghpatexpress.LessEqualEquation`.
...
/// An equation that constrains a pattern value to be less than or equal to one of the values
/// its right-hand pattern expression can take on.
///
/// Models `ghidra.pcodeCPort.slghpatexpress.LessEqualEquation`; see
/// [`GreaterEqualEquation`](super::greater_equal_equation::GreaterEqualEquation) for why this
/// is exposed as a trait.
pub trait LessEqualEquation: Send + Sync {
    ...
    /// Returns the token pattern computed by the most recent [`LessEqualEquation::gen_pattern`]
    /// call, if any.
    fn get_token_pattern(&self) -> Option<&dyn TokenPattern>;
}

/// The concrete `ghidra.pcodeCPort.slghpatexpress.LessEqualEquation`.
pub struct LessEqualEquationImpl {
    equation: ValExpressEquation,
    token_pattern: Option<Box<dyn TokenPattern>>,
}

impl LessEqualEquationImpl {
    /// Creates a new less-or-equal-constraint equation from its left- and right-hand operands.
...
impl LessEqualEquation for LessEqualEquationImpl {
    fn gen_pattern(&mut self) {
        let pattern = gen_comparison_pattern(
            ...
            "Less than or equal constraint is impossible to match",
        );
...
mod tests {
    ...
    #[test]
    fn gen_pattern_matches_when_some_lhsval_is_le_a_value() {
        ...
        let mut equation = LessEqualEquationImpl::new(loc(), lhs, rhs);
    ...
    #[test]
    #[should_panic(expected = "Less than or equal constraint is impossible to match")]
    fn gen_pattern_panics_when_every_lhsval_exceeds_every_value() {
    ...
    fn get_lhs_and_rhs_expose_operands() {
        ...
        let equation = LessEqualEquationImpl::new(loc(), lhs, rhs);
