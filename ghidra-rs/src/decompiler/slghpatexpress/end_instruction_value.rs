//! Models `ghidra.pcodeCPort.slghpatexpress.EndInstructionValue`.

use crate::decompiler::slghpattern::Pattern;
use crate::decompiler::slghpatexpress::{PatternValue, TokenPattern};
use crate::program::model::pcode::encoder::Encoder;
use crate::program::model::pcode::ids::ELEM_END_EXP;
use crate::sleigh::grammar::Location;
use std::io;

/// A minimal [`TokenPattern`] implementation for end instruction values.
/// Returns trivial patterns that just hold a location.
struct EndInstructionTokenPattern {
    location: Location,
    left_ellipsis: bool,
    right_ellipsis: bool,
}

impl EndInstructionTokenPattern {
...
impl TokenPattern for EndInstructionTokenPattern {
    ...
    fn do_and(&self, tokpat: &dyn TokenPattern) -> Box<dyn TokenPattern> {
        Box::new(EndInstructionTokenPattern::new(self.location.clone()))
    }
...
/// Represents the end instruction address in a sleigh pattern expression.
///
/// A pattern value that has fixed min/max bounds of 0, and generates trivial token patterns.
/// Its encode method writes the ELEM_END_EXP element.
///
/// Models `ghidra.pcodeCPort.slghpatexpress.EndInstructionValue`.
#[derive(Clone)]
...
impl EndInstructionValue {
    /// Creates a new end instruction value with the given location.
    pub fn new(location: Location) -> Self {
        Self { location }
    }

    /// Encodes this end instruction value to the given encoder.
    pub fn encode(&self, encoder: &mut dyn Encoder) -> io::Result<()> {
        encoder.open_element(ELEM_END_EXP)?;
        encoder.close_element(ELEM_END_EXP)?;
        Ok(())
    }
}

impl crate::decompiler::slghpatexpress::PatternExpression for EndInstructionValue {
...
impl crate::decompiler::slghpatexpress::PatternValue for EndInstructionValue {
...
mod tests {
    ...
    fn new_creates_with_location() {
        ...
        let value = EndInstructionValue::new(location.clone());
    ...
    fn min_value_returns_zero() {
        let location = Location::new("test.sleigh", 1);
        let value = EndInstructionValue::new(location);
