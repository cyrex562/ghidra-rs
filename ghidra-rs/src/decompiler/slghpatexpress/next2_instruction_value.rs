//! Models `ghidra.pcodeCPort.slghpatexpress.Next2InstructionValue`.

use crate::decompiler::slghpattern::Pattern;
use crate::decompiler::slghpatexpress::{PatternValue, TokenPattern};
use crate::program::model::pcode::encoder::Encoder;
use crate::program::model::pcode::ids::ELEM_NEXT2_EXP;
use crate::sleigh::grammar::Location;
use std::io;

/// A minimal [`TokenPattern`] implementation for next2 instruction values.
/// Returns trivial patterns that just hold a location.
struct Next2InstructionTokenPattern {
    location: Location,
    left_ellipsis: bool,
    right_ellipsis: bool,
}

impl Next2InstructionTokenPattern {
...
impl TokenPattern for Next2InstructionTokenPattern {
    ...
    fn do_and(&self, tokpat: &dyn TokenPattern) -> Box<dyn TokenPattern> {
        Box::new(Next2InstructionTokenPattern::new(self.location.clone()))
    }
...
/// Represents the next2 instruction address in a sleigh pattern expression.
///
/// A pattern value that has fixed min/max bounds of 0, and generates trivial token patterns.
/// Its encode method writes the ELEM_NEXT2_EXP element.
///
/// Models `ghidra.pcodeCPort.slghpatexpress.Next2InstructionValue`.
#[derive(Clone)]
...
impl Next2InstructionValue {
    /// Creates a new next2 instruction value with the given location.
    pub fn new(location: Location) -> Self {
        Self { location }
    }

    /// Encodes this next2 instruction value to the given encoder.
    pub fn encode(&self, encoder: &mut dyn Encoder) -> io::Result<()> {
        encoder.open_element(ELEM_NEXT2_EXP)?;
        encoder.close_element(ELEM_NEXT2_EXP)?;
        Ok(())
    }
}

impl crate::decompiler::slghpatexpress::PatternExpression for Next2InstructionValue {
...
impl crate::decompiler::slghpatexpress::PatternValue for Next2InstructionValue {
...
mod tests {
    ...
    fn new_creates_with_location() {
        ...
        let value = Next2InstructionValue::new(location.clone());
    ...
    fn min_value_returns_zero() {
        let location = Location::new("test.sleigh", 1);
        let value = Next2InstructionValue::new(location);
