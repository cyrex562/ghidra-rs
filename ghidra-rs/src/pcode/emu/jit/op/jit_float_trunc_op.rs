//! A use-def node for a TRUNC p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitFloatTruncOp`.
...
/// A use-def node for the [`OpCode::FloatTrunc`] p-code operation.
///
/// Port of the Java record `JitFloatTruncOp(PcodeOp op, JitOutVar out, JitVal u) implements
/// JitFloatUnOp`.
///
/// # Note
///
/// Java's record overrides `type()` to return `JitTypeBehavior::Integer` even though it implements
/// `JitFloatUnOp` (whose `uType()`/inherited default would otherwise suggest
/// `JitTypeBehavior::Float`-typed output): the operand is float-typed but the result is not, so the
/// override is deliberate, not a bug -- reproduced faithfully here in `JitDefOp::type_`.
///
...
#[derive(Clone)]
pub struct JitFloatTruncOp {
    op: PcodeOp,
    out: Arc<dyn JitOutVar>,
    u: Arc<dyn JitVal>,
}

impl JitFloatTruncOp {
...
impl JitOp for JitFloatTruncOp {
...
impl JitDefOp for JitFloatTruncOp {
...
impl JitUnOp for JitFloatTruncOp {
...
impl JitFloatUnOp for JitFloatTruncOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::FloatTrunc, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitFloatTruncOp::new(make_op(), out, u);
        assert_eq!(op.op().opcode, OpCode::FloatTrunc);
    }
