//! A use-def node for a INT_OR p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitIntOrOp`.
...
/// A use-def node for the [`OpCode::IntOr`] p-code operation.
///
/// Port of the Java record `JitIntOrOp(PcodeOp op, JitOutVar out, JitVal l, JitVal r)
...
impl JitIntBinOp for JitIntOrOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::IntOr, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitIntOrOp::new(make_op(), out, l, r);
        assert_eq!(op.op().opcode, OpCode::IntOr);
    }
