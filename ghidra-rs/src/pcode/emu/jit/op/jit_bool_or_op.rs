//! A use-def node for a BOOL_OR p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitBoolOrOp`.
...
/// A use-def node for the [`OpCode::BoolOr`] p-code operation.
///
/// Port of the Java record `JitBoolOrOp(PcodeOp op, JitOutVar out, JitVal l, JitVal r)
...
impl JitBoolBinOp for JitBoolOrOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::BoolOr, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitBoolOrOp::new(make_op(), out, l, r);
        assert_eq!(op.op().opcode, OpCode::BoolOr);
    }
