//! A use-def node for a INT_SUB p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitIntSubOp`.
...
/// A use-def node for the [`OpCode::IntSub`] p-code operation.
///
/// Port of the Java record `JitIntSubOp(PcodeOp op, JitOutVar out, JitVal l, JitVal r)
...
impl JitIntBinOp for JitIntSubOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::IntSub, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitIntSubOp::new(make_op(), out, l, r);
        assert_eq!(op.op().opcode, OpCode::IntSub);
    }
