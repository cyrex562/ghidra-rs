//! A use-def node for a INT_XOR p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitIntXorOp`.
...
/// A use-def node for the [`OpCode::IntXor`] p-code operation.
///
/// Port of the Java record `JitIntXorOp(PcodeOp op, JitOutVar out, JitVal l, JitVal r)
...
impl JitIntBinOp for JitIntXorOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::IntXor, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitIntXorOp::new(make_op(), out, l, r);
        assert_eq!(op.op().opcode, OpCode::IntXor);
    }
