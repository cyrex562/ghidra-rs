//! A use-def node for a FLOAT_SQRT p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitFloatSqrtOp`.
...
/// A use-def node for the [`OpCode::FloatSqrt`] p-code operation.
///
/// Port of the Java record `JitFloatSqrtOp(PcodeOp op, JitOutVar out, JitVal u) implements
/// JitFloatUnOp`.
///
...
impl JitDefOp for JitFloatSqrtOp {
...
impl JitUnOp for JitFloatSqrtOp {
...
impl JitFloatUnOp for JitFloatSqrtOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::FloatSqrt, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitFloatSqrtOp::new(make_op(), out, u);
        assert_eq!(op.op().opcode, OpCode::FloatSqrt);
    }
