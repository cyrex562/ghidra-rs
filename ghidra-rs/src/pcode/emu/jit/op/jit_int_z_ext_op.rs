//! A use-def node for a INT_ZEXT p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitIntZExtOp`.
...
/// A use-def node for the [`OpCode::IntZext`] p-code operation.
///
/// Port of the Java record `JitIntZExtOp(PcodeOp op, JitOutVar out, JitVal u) implements
/// JitIntUnOp`.
///
...
impl JitDefOp for JitIntZExtOp {
...
impl JitIntUnOp for JitIntZExtOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::IntZext, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitIntZExtOp::new(make_op(), out, u);
        assert_eq!(op.op().opcode, OpCode::IntZext);
    }
