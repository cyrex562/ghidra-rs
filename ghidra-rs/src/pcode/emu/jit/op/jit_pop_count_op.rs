//! A use-def node for a POPCOUNT p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitPopCountOp`.
...
/// A use-def node for the [`OpCode::Popcount`] p-code operation.
///
/// Port of the Java record `JitPopCountOp(PcodeOp op, JitOutVar out, JitVal u) implements
/// JitIntUnOp`.
///
...
impl JitDefOp for JitPopCountOp {
...
impl JitUnOp for JitPopCountOp {
...
impl JitIntUnOp for JitPopCountOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::Popcount, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitPopCountOp::new(make_op(), out, u);
        assert_eq!(op.op().opcode, OpCode::Popcount);
    }
