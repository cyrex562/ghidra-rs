//! A use-def node for a COPY p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitCopyOp`.
...
/// A use-def node for the [`OpCode::Copy`] p-code operation.
///
/// Port of the Java record `JitCopyOp(PcodeOp op, JitOutVar out, JitVal u) implements JitUnOp`,
/// which overrides both `uType()` and `type()` to [`JitTypeBehavior::Copy`] directly (it
/// implements `JitUnOp` itself, not one of the `JitIntUnOp`/`JitFloatUnOp`/`JitBoolUnOp`
/// subtraits, since a copy imposes no type requirement of its own -- it just repeats whatever
/// interpretation the input already has).
///
...
impl JitOp for JitCopyOp {
    /// Port of `JitUnOp.typeFor(int)`: position 0 is `uType()` ([`JitTypeBehavior::Copy`]),
    /// anything else throws `AssertionError`.
    fn type_for(&self, position: i32) -> JitTypeBehavior {
        match position {
            0 => JitTypeBehavior::Copy,
...
impl JitDefOp for JitCopyOp {
    ...
    /// Port of `type()`, overridden to [`JitTypeBehavior::Copy`].
    fn type_(&self) -> JitTypeBehavior {
        JitTypeBehavior::Copy
    }
}

impl JitUnOp for JitCopyOp {
    fn u(&self) -> Arc<dyn JitVal> {
        Arc::clone(&self.u)
    }

    /// Port of `uType()`, overridden to [`JitTypeBehavior::Copy`].
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::Copy, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitCopyOp::new(make_op(), out, u);
        assert_eq!(op.op().opcode, OpCode::Copy);
    }
    ...
    /// Java: `JitCopyOp` overrides both `uType()` and `type()` to `JitTypeBehavior.COPY`, unlike
    /// every other `JitUnOp` implementor (which takes its type from a subtrait such as
    /// `JitIntUnOp`/`JitFloatUnOp`). This is deliberate: a copy carries no type requirement of its
    /// own.
    #[test]
    fn u_type_and_type_are_copy() {
        let out = Arc::new(MockOutVar) as Arc<dyn JitOutVar>;
        let u = Arc::new(MockVal) as Arc<dyn JitVal>;
        let op = JitCopyOp::new(make_op(), out, u);
        assert_eq!(JitUnOp::u_type(&op), JitTypeBehavior::Copy);
        assert_eq!(JitDefOpTrait::type_(&op), JitTypeBehavior::Copy);
    }
    ...
    fn type_for_dispatches_to_u_type() {
        ...
        let u = Arc::new(MockVal) as Arc<dyn JitVal>;
        let op = JitCopyOp::new(make_op(), out, u);
        assert_eq!(op.type_for(0), JitTypeBehavior::Copy);
    }
