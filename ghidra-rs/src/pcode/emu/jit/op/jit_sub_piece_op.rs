//! A use-def node for a SUBPIECE p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitSubPieceOp`.
...
use super::{JitIntUnOp, JitUnOp};

/// A use-def node for the [`OpCode::Subpiece`] p-code operation.
///
/// Port of the Java record `JitSubPieceOp(PcodeOp op, JitOutVar out, JitVal u, int offset)
/// implements JitIntUnOp`.
///
/// # Note
///
/// This is distinct from
/// [`JitSynthSubPieceOp`](crate::pcode::seam_stubs::JitSynthSubPieceOp) (a synthetic node
/// generated later during data-flow analysis when memory access patterns require an implicit
/// subpiece); `JitSubPieceOp` wraps an actual `SUBPIECE` p-code op emitted by the decompiler.
///
/// # Differences from Java
///
/// As with the other unary op nodes (see
/// [`JitBinOp`](crate::pcode::emu::jit::op::jit_bin_op::JitBinOp)'s docs for why), Rust cannot
/// express `JitIntUnOp`'s inherited defaults, so `link`/`unlink`/`inputs`/`type_for` are written
/// out directly here, mirroring `JitDefOp.link()` (`out().setDefinition(this)`) followed by
/// `JitUnOp.link()`'s `u().addUse(this, 0)` (and the symmetric `unlink()`).
#[derive(Clone)]
pub struct JitSubPieceOp {
    op: PcodeOp,
...
impl JitSubPieceOp {
    ...
    pub fn new(op: PcodeOp, out: Arc<dyn JitOutVar>, u: Arc<dyn JitVal>, offset: i32) -> Self {
        Self { op, out, u, offset }
    }
    ...
    /// Port of the record accessor `offset()`: the number of bytes to shift right.
    pub fn offset(&self) -> i32 {
        self.offset
    }
}

impl JitOp for JitSubPieceOp {
...
impl JitDefOp for JitSubPieceOp {
...
impl JitUnOp for JitSubPieceOp {
...
impl JitIntUnOp for JitSubPieceOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::Subpiece, seqnum, vec![], None)
    }
    ...
    #[test]
    fn has_fields_including_offset() {
        let out = Arc::new(MockOutVar) as Arc<dyn JitOutVar>;
        let u = Arc::new(MockVal) as Arc<dyn JitVal>;
        let op = JitSubPieceOp::new(make_op(), out, u, 2);
        assert_eq!(op.op().opcode, OpCode::Subpiece);
    ...
    fn implements_expected_traits() {
        let out = Arc::new(MockOutVar) as Arc<dyn JitOutVar>;
        let u = Arc::new(MockVal) as Arc<dyn JitVal>;
        let op = JitSubPieceOp::new(make_op(), out, u, 0);
        let _: &dyn JitIntUnOp = &op;
    ...
    #[test]
    fn u_type_and_type_are_integer() {
        let out = Arc::new(MockOutVar) as Arc<dyn JitOutVar>;
        let u = Arc::new(MockVal) as Arc<dyn JitVal>;
        let op = JitSubPieceOp::new(make_op(), out, u, 1);
        assert_eq!(JitUnOp::u_type(&op), JitTypeBehavior::Integer);
        assert_eq!(JitDefOpTrait::type_(&op), JitTypeBehavior::Integer);
    }
