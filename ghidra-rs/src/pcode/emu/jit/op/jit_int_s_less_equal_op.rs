//! A use-def node for a INT_SLESSEQUAL p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitIntSLessEqualOp`.
...
use super::{JitIntBinOp, JitIntTestOp};

/// A use-def node for the [`OpCode::IntSlessEqual`] p-code operation.
///
/// Port of the Java record `JitIntSLessEqualOp(PcodeOp op, JitOutVar out, JitVal l, JitVal r)
/// implements JitIntTestOp`.
///
/// # Differences from Java
///
/// Java's record automatically implements `JitIntTestOp`'s inherited default
...
impl JitOp for JitIntSLessEqualOp {
...
impl JitDefOp for JitIntSLessEqualOp {
...
impl JitBinOp for JitIntSLessEqualOp {
...
impl JitIntBinOp for JitIntSLessEqualOp {}
impl JitIntTestOp for JitIntSLessEqualOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::IntSlessEqual, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitIntSLessEqualOp::new(make_op(), out, l, r);
        assert_eq!(op.op().opcode, OpCode::IntSlessEqual);
    }
    ...
    fn implements_expected_traits() {
        ...
        let r = Arc::new(MockVal) as Arc<dyn JitVal>;
        let op = JitIntSLessEqualOp::new(make_op(), out, l, r);
        let _: &dyn JitIntBinOp = &op;
        let _: &dyn JitIntTestOp = &op;
