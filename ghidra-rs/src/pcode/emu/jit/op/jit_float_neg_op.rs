//! A use-def node for a FLOAT_NEG p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitFloatNegOp`.
...
/// A use-def node for the [`OpCode::FloatNeg`] p-code operation.
///
/// Port of the Java record `JitFloatNegOp(PcodeOp op, JitOutVar out, JitVal u) implements
/// JitFloatUnOp`.
///
...
impl JitDefOp for JitFloatNegOp {
...
impl JitUnOp for JitFloatNegOp {
...
impl JitFloatUnOp for JitFloatNegOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::FloatNeg, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitFloatNegOp::new(make_op(), out, u);
        assert_eq!(op.op().opcode, OpCode::FloatNeg);
    }
