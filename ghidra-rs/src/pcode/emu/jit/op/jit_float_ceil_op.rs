//! A use-def node for a CEIL p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitFloatCeilOp`.
...
/// A use-def node for the [`OpCode::FloatCeil`] p-code operation.
///
/// Port of the Java record `JitFloatCeilOp(PcodeOp op, JitOutVar out, JitVal u) implements
/// JitFloatUnOp`.
///
...
impl JitDefOp for JitFloatCeilOp {
...
impl JitUnOp for JitFloatCeilOp {
...
impl JitFloatUnOp for JitFloatCeilOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::FloatCeil, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitFloatCeilOp::new(make_op(), out, u);
        assert_eq!(op.op().opcode, OpCode::FloatCeil);
    }
