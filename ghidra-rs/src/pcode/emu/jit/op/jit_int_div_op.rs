//! A use-def node for a INT_DIV p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitIntDivOp`.
...
/// A use-def node for the [`OpCode::IntDiv`] p-code operation.
///
/// Port of the Java record `JitIntDivOp(PcodeOp op, JitOutVar out, JitVal l, JitVal r)
...
impl JitIntBinOp for JitIntDivOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::IntDiv, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitIntDivOp::new(make_op(), out, l, r);
        assert_eq!(op.op().opcode, OpCode::IntDiv);
    }
