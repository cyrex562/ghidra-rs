//! A use-def node for a INT_REM p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitIntRemOp`.
...
/// A use-def node for the [`OpCode::IntRem`] p-code operation.
///
/// Port of the Java record `JitIntRemOp(PcodeOp op, JitOutVar out, JitVal l, JitVal r)
...
impl JitIntBinOp for JitIntRemOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::IntRem, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitIntRemOp::new(make_op(), out, l, r);
        assert_eq!(op.op().opcode, OpCode::IntRem);
    }
