//! A use-def node for a INT_ADD p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitIntAddOp`.
...
/// A use-def node for the [`OpCode::IntAdd`] p-code operation.
///
/// Port of the Java record `JitIntAddOp(PcodeOp op, JitOutVar out, JitVal l, JitVal r)
...
impl JitIntBinOp for JitIntAddOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::IntAdd, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitIntAddOp::new(make_op(), out, l, r);
        assert_eq!(op.op().opcode, OpCode::IntAdd);
    }
