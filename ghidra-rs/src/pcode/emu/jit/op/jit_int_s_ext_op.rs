//! A use-def node for a INT_SEXT p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitIntSExtOp`.
...
/// A use-def node for the [`OpCode::IntSext`] p-code operation.
///
/// Port of the Java record `JitIntSExtOp(PcodeOp op, JitOutVar out, JitVal u) implements
/// JitIntUnOp`.
///
...
impl JitDefOp for JitIntSExtOp {
...
impl JitIntUnOp for JitIntSExtOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::IntSext, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitIntSExtOp::new(make_op(), out, u);
        assert_eq!(op.op().opcode, OpCode::IntSext);
    }
