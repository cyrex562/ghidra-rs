//! A use-def node for a INT_NEGATE p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitIntNegateOp`.
...
/// A use-def node for the [`OpCode::IntNegate`] p-code operation.
///
/// Port of the Java record `JitIntNegateOp(PcodeOp op, JitOutVar out, JitVal u) implements
/// JitIntUnOp`.
///
...
/// `JitIntUnOp`'s inherited defaults, so `link`/`unlink`/`inputs`/`type_for` are written out
...
impl JitDefOp for JitIntNegateOp {
...
impl JitUnOp for JitIntNegateOp {
...
impl JitIntUnOp for JitIntNegateOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::IntNegate, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitIntNegateOp::new(make_op(), out, u);
        assert_eq!(op.op().opcode, OpCode::IntNegate);
    }
