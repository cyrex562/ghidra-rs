//! A use-def node for a LZCOUNT p-code operation.
//!
//! Port of `ghidra.pcode.emu.jit.op.JitLzCountOp`.
...
/// A use-def node for the [`OpCode::Lzcount`] p-code operation.
///
/// Port of the Java record `JitLzCountOp(PcodeOp op, JitOutVar out, JitVal u) implements
/// JitIntUnOp`.
///
...
impl JitDefOp for JitLzCountOp {
...
impl JitIntUnOp for JitLzCountOp {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::Lzcount, seqnum, vec![], None)
    }
    ...
    fn has_fields() {
        ...
        let op = JitLzCountOp::new(make_op(), out, u);
        assert_eq!(op.op().opcode, OpCode::Lzcount);
    }
