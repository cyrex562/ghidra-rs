//! The generator for a `int_and` op.
//!
//! Port of `ghidra.pcode.emu.jit.gen.op.IntAndOpGen`.
//!
//! # Differences from Java
//!
//! - Java's singleton `enum IntAndOpGen implements IntBitwiseBinOpGen<JitIntAndOp> { GEN; }`
...
use crate::pcode::emu::jit::op::JitIntAndOp;
use crate::pcode::seam_stubs::{JitCodeGenerator, OpGen, Scope};

/// The generator for a [`JitIntAndOp`] (`int_and`).
///
/// Port of `ghidra.pcode.emu.jit.gen.op.IntAndOpGen`.
///
/// This uses the bitwise binary operator and emits `iand` or `land` depending on the type.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum IntAndOpGen {
    /// The generator singleton.
    Gen,
}

impl IntAndOpGen {
    /// The singleton instance of `IntAndOpGen`.
    pub const GEN: Self = IntAndOpGen::Gen;
}

impl OpGen<JitIntAndOp> for IntAndOpGen {}

impl BinOpGen<JitIntAndOp> for IntAndOpGen {
    fn is_signed(&self) -> bool {
        self.int_bitwise_bin_op_gen_is_signed()
    }
}

impl IntOpBinOpGen<JitIntAndOp> for IntAndOpGen {
...
impl IntBitwiseBinOpGen<JitIntAndOp> for IntAndOpGen {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::IntAnd, SequenceNumber::new(addr, 0), vec![], None)
    }
    ...
    #[test]
    fn gen_implements_op_gen_for_jit_int_and_op() {
        // Java: `enum IntAndOpGen implements IntBitwiseBinOpGen<JitIntAndOp> { GEN; }`
        fn assert_is_op_gen<T: JitOp>(_gen: &impl OpGen<T>) {}
        assert_is_op_gen::<JitIntAndOp>(&IntAndOpGen::Gen);
    }

    #[test]
    fn gen_is_zero_sized_singleton() {
        assert_eq!(std::mem::size_of::<IntAndOpGen>(), 0);
        assert_eq!(IntAndOpGen::GEN, IntAndOpGen::Gen);
    }

    #[test]
    fn is_signed_is_false_like_bitwise_operators() {
        // Java: `IntBitwiseBinOpGen.isSigned()` unconditionally returns `false`.
        assert!(!BinOpGen::is_signed(&IntAndOpGen::GEN));
        assert_eq!(IntAndOpGen::GEN.ext(), Ext::Zero);
    }
    ...
    fn op_for_int_preserves_the_stack_tail() {
        ...
        let result: Emitter<Ent<Tail, TInt>> = IntAndOpGen::GEN.op_for_int(em, IntJitType::I4);
    ...
    fn op_for_long_preserves_the_stack_tail() {
        ...
        let result: Emitter<Ent<Tail, TLong>> = IntAndOpGen::GEN.op_for_long(em, LongJitType::I8);
        assert!(result.local_variables().is_empty());
    }
    ...
    #[test]
    fn int_and_op_gen_extends_int_bitwise_bin_op_gen_like_java_interface() {
        // Java: `interface IntAndOpGen implements IntBitwiseBinOpGen<JitIntAndOp>`.
        fn assert_is_int_bitwise<G: IntBitwiseBinOpGen<JitIntAndOp>>(_gen: &G) {}
        assert_is_int_bitwise(&IntAndOpGen::GEN);
    }
}
