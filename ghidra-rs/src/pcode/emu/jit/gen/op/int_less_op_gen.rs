//! The generator for a `int_less` op.
//!
//! Port of `ghidra.pcode.emu.jit.gen.op.IntLessOpGen`.
//!
//! # Differences from Java
//!
//! - Java's singleton `enum IntLessOpGen implements IntCompareBinOpGen<JitIntLessOp> { GEN; }`
...
//!   with `Op::iflt` as the conditional-jump opcode. `Op::iflt` itself is not yet ported (see
...
use crate::pcode::emu::jit::op::JitIntLessOp;
use crate::pcode::seam_stubs::{JitCodeGenerator, Scope};
use crate::pcode::emu::jit::gen::op::op_gen::OpGen;
/// The generator for a [`JitIntLessOp`] (`int_less`).
///
/// Port of `ghidra.pcode.emu.jit.gen.op.IntLessOpGen`.
///
/// This uses the (unsigned) integer comparison operator generator and simply emits `iflt`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum IntLessOpGen {
    /// The generator singleton.
    Gen,
}

impl IntLessOpGen {
    /// The singleton instance of `IntLessOpGen`.
    pub const GEN: Self = IntLessOpGen::Gen;
}

impl OpGen<JitIntLessOp> for IntLessOpGen {}

impl BinOpGen<JitIntLessOp> for IntLessOpGen {
    fn is_signed(&self) -> bool {
        false
    }
}

impl IntPredBinOpGen<JitIntLessOp> for IntLessOpGen {
...
impl IntCompareBinOpGen<JitIntLessOp> for IntLessOpGen {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::IntLess, SequenceNumber::new(addr, 0), vec![], None)
    }
    ...
    #[test]
    fn gen_implements_op_gen_for_jit_int_less_op() {
        // Java: `enum IntLessOpGen implements IntCompareBinOpGen<JitIntLessOp> { GEN; }`
        fn assert_is_op_gen<T: JitOp>(_gen: &impl OpGen<T>) {}
        assert_is_op_gen::<JitIntLessOp>(&IntLessOpGen::Gen);
    }

    #[test]
    fn gen_is_zero_sized_singleton() {
        assert_eq!(std::mem::size_of::<IntLessOpGen>(), 0);
        assert_eq!(IntLessOpGen::GEN, IntLessOpGen::Gen);
    }

    #[test]
    fn is_signed_is_false() {
        // Java: `IntLessOpGen.isSigned()` returns `false` (unsigned comparison).
        assert!(!BinOpGen::is_signed(&IntLessOpGen::GEN));
        assert_eq!(IntLessOpGen::GEN.ext(), Ext::Zero);
    }
    ...
    fn op_for_int_reduces_two_ints_to_one_boolean() {
        ...
        let result: Emitter<Ent<Tail, TInt>> = IntLessOpGen::GEN.op_for_int(em, IntJitType::I4);
    ...
    fn op_for_long_reduces_two_longs_to_one_boolean() {
        ...
        let result: Emitter<Ent<Tail, TInt>> =
            IntLessOpGen::GEN.op_for_long(em, LongJitType::I8);
        assert!(result.local_variables().is_empty());
    }
    ...
    #[test]
    fn int_less_op_gen_extends_int_compare_bin_op_gen_like_java_interface() {
        fn assert_is_int_compare<G: IntCompareBinOpGen<JitIntLessOp>>(_gen: &G) {}
        assert_is_int_compare(&IntLessOpGen::GEN);
    }
}
