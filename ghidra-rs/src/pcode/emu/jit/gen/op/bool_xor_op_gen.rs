//! The generator for a `bool_xor` op.
//!
//! Port of `ghidra.pcode.emu.jit.gen.op.BoolXorOpGen`.
//!
//! # Differences from Java
//!
//! - Java's singleton `enum BoolXorOpGen implements IntBitwiseBinOpGen<JitBoolXorOp> { GEN; }`
...
//! just like `OpBehaviorBoolXor`. Thus, this is identical to
//! [`IntXorOpGen`](super::int_xor_op_gen::IntXorOpGen). Because having bits other than the least
//! significant set in the inputs is "undefined behavior," Java notes this could technically be
//! optimized to only XOR the least significant leg when dealing with mp-ints -- an optimization
...
use crate::pcode::emu::jit::op::JitBoolXorOp;
use crate::pcode::seam_stubs::{JitCodeGenerator, OpGen, Scope};

/// The generator for a [`JitBoolXorOp`] (`bool_xor`).
///
/// Port of `ghidra.pcode.emu.jit.gen.op.BoolXorOpGen`.
///
/// This uses the bitwise binary operator and emits `ixor` or `lxor` depending on the type.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BoolXorOpGen {
    /// The generator singleton.
    Gen,
}

impl BoolXorOpGen {
    /// The singleton instance of `BoolXorOpGen`.
    pub const GEN: Self = BoolXorOpGen::Gen;
}

impl OpGen<JitBoolXorOp> for BoolXorOpGen {}

impl BinOpGen<JitBoolXorOp> for BoolXorOpGen {
    fn is_signed(&self) -> bool {
        self.int_bitwise_bin_op_gen_is_signed()
    }
}

impl IntOpBinOpGen<JitBoolXorOp> for BoolXorOpGen {
...
impl IntBitwiseBinOpGen<JitBoolXorOp> for BoolXorOpGen {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::BoolXor, SequenceNumber::new(addr, 0), vec![], None)
    }
    ...
    #[test]
    fn gen_implements_op_gen_for_jit_bool_xor_op() {
        // Java: `enum BoolXorOpGen implements IntBitwiseBinOpGen<JitBoolXorOp> { GEN; }`
        fn assert_is_op_gen<T: JitOp>(_gen: &impl OpGen<T>) {}
        assert_is_op_gen::<JitBoolXorOp>(&BoolXorOpGen::Gen);
    }

    #[test]
    fn gen_is_zero_sized_singleton() {
        assert_eq!(std::mem::size_of::<BoolXorOpGen>(), 0);
        assert_eq!(BoolXorOpGen::GEN, BoolXorOpGen::Gen);
    }

    #[test]
    fn is_signed_is_false_like_bitwise_operators() {
        // Java: `IntBitwiseBinOpGen.isSigned()` unconditionally returns `false`.
        assert!(!BinOpGen::is_signed(&BoolXorOpGen::GEN));
        assert_eq!(BoolXorOpGen::GEN.ext(), Ext::Zero);
    }
    ...
    fn op_for_int_preserves_the_stack_tail() {
        ...
        let result: Emitter<Ent<Tail, TInt>> = BoolXorOpGen::GEN.op_for_int(em, IntJitType::I4);
    ...
    fn op_for_long_preserves_the_stack_tail() {
        ...
        let result: Emitter<Ent<Tail, TLong>> = BoolXorOpGen::GEN.op_for_long(em, LongJitType::I8);
        assert!(result.local_variables().is_empty());
    }
    ...
    #[test]
    fn bool_xor_op_gen_extends_int_bitwise_bin_op_gen_like_java_interface() {
        // Java: `interface BoolXorOpGen implements IntBitwiseBinOpGen<JitBoolXorOp>`.
        fn assert_is_int_bitwise<G: IntBitwiseBinOpGen<JitBoolXorOp>>(_gen: &G) {}
        assert_is_int_bitwise(&BoolXorOpGen::GEN);
    }
}
