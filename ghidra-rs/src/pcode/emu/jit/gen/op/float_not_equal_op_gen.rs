//! The generator for a `float_notequal` op.
//!
//! Port of `ghidra.pcode.emu.jit.gen.op.FloatNotEqualOpGen`.
//!
//! # Differences from Java
//!
//! - Java's singleton `enum FloatNotEqualOpGen implements FloatCompareBinOpGen<JitFloatNotEqualOp>
...
//!   `Op::ifne`. The not-yet-ported `Op` (JVM opcode helper namespace) means none of these opcodes
...
/// depending on the type and then `ifne`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FloatNotEqualOpGen {
    /// The generator singleton.
    Gen,
}

impl FloatNotEqualOpGen {
    /// The singleton instance of `FloatNotEqualOpGen`.
    pub const GEN: Self = FloatNotEqualOpGen::Gen;
}

impl OpGen<JitFloatNotEqualOp> for FloatNotEqualOpGen {}

impl BinOpGen<JitFloatNotEqualOp> for FloatNotEqualOpGen {
    fn is_signed(&self) -> bool {
        self.float_compare_bin_op_gen_is_signed()
    }
}

impl FloatCompareBinOpGen<JitFloatNotEqualOp> for FloatNotEqualOpGen {
    ...
    fn op_for_cond_jump<N: Next>(&self, em: Emitter<Ent<N, TInt>>) -> LblEm<N, N> {
        // Op::ifne is not yet ported; see the module docs.
        Lbl::place(em.recast())
    }
}
...
mod tests {
    ...
    #[test]
    fn gen_implements_op_gen_for_jit_float_not_equal_op() {
        // Java: `enum FloatNotEqualOpGen implements FloatCompareBinOpGen<JitFloatNotEqualOp> { GEN; }`
        fn assert_is_op_gen<T: JitOp>(_gen: &impl OpGen<T>) {}
        assert_is_op_gen::<JitFloatNotEqualOp>(&FloatNotEqualOpGen::Gen);
    }

    #[test]
    fn gen_is_zero_sized_singleton() {
        assert_eq!(std::mem::size_of::<FloatNotEqualOpGen>(), 0);
        assert_eq!(FloatNotEqualOpGen::GEN, FloatNotEqualOpGen::Gen);
    }

    #[test]
    fn is_signed_is_false_like_float_operators() {
        assert!(!BinOpGen::is_signed(&FloatNotEqualOpGen::GEN));
        assert_eq!(FloatNotEqualOpGen::GEN.ext(), Ext::Zero);
    }
    ...
    fn op_for_float_cmp_yields_an_int_result() {
        ...
        let result: Emitter<Ent<Tail, TInt>> = FloatNotEqualOpGen::GEN.op_for_float_cmp(em);
    ...
    fn op_for_double_cmp_yields_an_int_result() {
        ...
        let result: Emitter<Ent<Tail, TInt>> = FloatNotEqualOpGen::GEN.op_for_double_cmp(em);
        assert!(result.local_variables().is_empty());
    }
    ...
    fn op_for_cond_jump_pops_the_comparison_result() {
        ...
        let LblEm { em: result, .. } = FloatNotEqualOpGen::GEN.op_for_cond_jump(em);
        let _: Emitter<Tail> = result;
    }

    #[test]
    fn float_not_equal_op_gen_extends_float_compare_bin_op_gen_like_java_interface() {
        fn assert_is_float_compare<G: FloatCompareBinOpGen<JitFloatNotEqualOp>>(_gen: &G) {}
        assert_is_float_compare(&FloatNotEqualOpGen::GEN);
    }
}
