//! The generator for a `int_or` op.
//!
//! Port of `ghidra.pcode.emu.jit.gen.op.IntOrOpGen`.
//!
//! # Differences from Java
//!
//! - Java's singleton `enum IntOrOpGen implements IntBitwiseBinOpGen<JitIntOrOp> { GEN; }` becomes
//!   a zero-sized unit-like enum with a single variant and a `GEN` constant, per the convention set
//!   by [`CopyOpGen`](super::copy_op_gen::CopyOpGen) and
...
impl IntOrOpGen {
    /// The singleton instance of `IntOrOpGen`.
    pub const GEN: Self = IntOrOpGen::Gen;
}

impl OpGen<JitIntOrOp> for IntOrOpGen {}

impl BinOpGen<JitIntOrOp> for IntOrOpGen {
    fn is_signed(&self) -> bool {
        self.int_bitwise_bin_op_gen_is_signed()
    }
}

impl IntOpBinOpGen<JitIntOrOp> for IntOrOpGen {
...
impl IntBitwiseBinOpGen<JitIntOrOp> for IntOrOpGen {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::IntOr, SequenceNumber::new(addr, 0), vec![], None)
    }
    ...
    #[test]
    fn gen_implements_op_gen_for_jit_int_or_op() {
        // Java: `enum IntOrOpGen implements IntBitwiseBinOpGen<JitIntOrOp> { GEN; }`
        fn assert_is_op_gen<T: JitOp>(_gen: &impl OpGen<T>) {}
        assert_is_op_gen::<JitIntOrOp>(&IntOrOpGen::Gen);
    }

    #[test]
    fn gen_is_zero_sized_singleton() {
        assert_eq!(std::mem::size_of::<IntOrOpGen>(), 0);
        assert_eq!(IntOrOpGen::GEN, IntOrOpGen::Gen);
    }

    #[test]
    fn is_signed_is_false_like_bitwise_operators() {
        assert!(!BinOpGen::is_signed(&IntOrOpGen::GEN));
        assert_eq!(IntOrOpGen::GEN.ext(), Ext::Zero);
    }
    ...
    fn op_for_int_preserves_the_stack_tail() {
        ...
        let result: Emitter<Ent<Tail, TInt>> = IntOrOpGen::GEN.op_for_int(em, IntJitType::I4);
    ...
    fn op_for_long_preserves_the_stack_tail() {
        ...
        let result: Emitter<Ent<Tail, TLong>> = IntOrOpGen::GEN.op_for_long(em, LongJitType::I8);
        assert!(result.local_variables().is_empty());
    }
    ...
    #[test]
    fn int_or_op_gen_extends_int_bitwise_bin_op_gen_like_java_interface() {
        fn assert_is_int_bitwise<G: IntBitwiseBinOpGen<JitIntOrOp>>(_gen: &G) {}
        assert_is_int_bitwise(&IntOrOpGen::GEN);
    }
}
