//! The generator for a `int_sless` op.
//!
//! Port of `ghidra.pcode.emu.jit.gen.op.IntSLessOpGen`.
//!
//! # Differences from Java
//!
//! - Java's singleton `enum IntSLessOpGen implements IntCompareBinOpGen<JitIntSLessOp> { GEN; }`
...
//! - `opForInt` delegates to [`IntCompareBinOpGen::gen_int_via_if_icmp`] with `Op::if_icmplt` as
//!   the conditional-jump opcode, and `opForLong` delegates to
//!   [`IntCompareBinOpGen::gen_long_via_lcmp_then_if`] with `Op::iflt`. Neither opcode is yet
...
use crate::pcode::emu::jit::op::JitIntSLessOp;
use crate::pcode::seam_stubs::{JitCodeGenerator, Scope};
use crate::pcode::emu::jit::gen::op::op_gen::OpGen;
/// The generator for a [`JitIntSLessOp`] (`int_sless`).
///
/// Port of `ghidra.pcode.emu.jit.gen.op.IntSLessOpGen`.
///
/// This uses the (signed) integer comparison operator generator and simply emits `if_icmplt` or
/// `iflt` depending on the type.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum IntSLessOpGen {
    /// The generator singleton.
    Gen,
}

impl IntSLessOpGen {
    /// The singleton instance of `IntSLessOpGen`.
    pub const GEN: Self = IntSLessOpGen::Gen;
}

impl OpGen<JitIntSLessOp> for IntSLessOpGen {}

impl BinOpGen<JitIntSLessOp> for IntSLessOpGen {
    fn is_signed(&self) -> bool {
        true
    }
}

impl IntPredBinOpGen<JitIntSLessOp> for IntSLessOpGen {
    fn op_for_int<N: Next>(
        &self,
        em: Emitter<Ent<Ent<N, TInt>, TInt>>,
        _type_: IntJitType,
    ) -> Emitter<Ent<N, TInt>> {
        // Op::if_icmplt is not yet ported; see the module docs.
...
impl IntCompareBinOpGen<JitIntSLessOp> for IntSLessOpGen {}
...
mod tests {
    ...
    fn make_op() -> PcodeOp {
        ...
        PcodeOp::new(OpCode::IntSless, SequenceNumber::new(addr, 0), vec![], None)
    }
    ...
    #[test]
    fn gen_implements_op_gen_for_jit_int_s_less_op() {
        // Java: `enum IntSLessOpGen implements IntCompareBinOpGen<JitIntSLessOp> { GEN; }`
        fn assert_is_op_gen<T: JitOp>(_gen: &impl OpGen<T>) {}
        assert_is_op_gen::<JitIntSLessOp>(&IntSLessOpGen::Gen);
    }

    #[test]
    fn gen_is_zero_sized_singleton() {
        assert_eq!(std::mem::size_of::<IntSLessOpGen>(), 0);
        assert_eq!(IntSLessOpGen::GEN, IntSLessOpGen::Gen);
    }

    #[test]
    fn is_signed_is_true() {
        // Java: `IntSLessOpGen.isSigned()` returns `true` (signed comparison).
        assert!(BinOpGen::is_signed(&IntSLessOpGen::GEN));
        assert_eq!(IntSLessOpGen::GEN.ext(), Ext::Sign);
    }
    ...
    fn op_for_int_reduces_two_ints_to_one_boolean_via_if_icmp() {
        ...
        let result: Emitter<Ent<Tail, TInt>> = IntSLessOpGen::GEN.op_for_int(em, IntJitType::I4);
    ...
    fn op_for_long_reduces_two_longs_to_one_boolean_via_lcmp() {
        ...
        let result: Emitter<Ent<Tail, TInt>> =
            IntSLessOpGen::GEN.op_for_long(em, LongJitType::I8);
        assert!(result.local_variables().is_empty());
    }
    ...
    #[test]
    fn int_s_less_op_gen_extends_int_compare_bin_op_gen_like_java_interface() {
        fn assert_is_int_compare<G: IntCompareBinOpGen<JitIntSLessOp>>(_gen: &G) {}
        assert_is_int_compare(&IntSLessOpGen::GEN);
    }
}
