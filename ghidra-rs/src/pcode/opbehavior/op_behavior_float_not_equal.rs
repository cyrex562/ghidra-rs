//! Port of `ghidra.pcode.opbehavior.OpBehaviorFloatNotEqual`.
...
/// FLOAT_NOTEQUAL p-code operation behavior: floating-point inequality (a one-byte boolean).
///
...
///
/// Corresponds to `ghidra.pcode.opbehavior.OpBehaviorFloatNotEqual`.
...
impl OpBehaviorFloatNotEqual {
    /// Construct a new `OpBehaviorFloatNotEqual` for [`OpCode::FloatNotEqual`].
    pub fn new() -> Self {
        Self { base: OpBehavior::new(OpCode::FloatNotEqual as i32) }
    }
}

impl Default for OpBehaviorFloatNotEqual {
    fn default() -> Self {
        Self::new()
    }
}

impl BinaryOpBehavior for OpBehaviorFloatNotEqual {
    fn opcode(&self) -> i32 {
        self.base.opcode()
    }
    ...
    fn evaluate_binary_i128(&self, _sizeout: i32, sizein: i32, in1: i128, in2: i128) -> i128 {
        let format = format_for(sizein);
        big_int_to_i128(&format.op_not_equal_big(&BigInt::from(in1), &BigInt::from(in2)))
    }
}
...
mod tests {
    ...
    fn opcode_matches_pcode_op() {
        assert_eq!(OpBehaviorFloatNotEqual::new().opcode(), OpCode::FloatNotEqual as i32);
    }

    #[test]
    fn evaluate_binary_long() {
        let op = OpBehaviorFloatNotEqual::new();
        let ff = ff(8);
        let e = |d: f64| ff.get_encoding(d);
        for (a, b, expect) in [(1.234, 1.234, 0), (-1.234, -1.234, 0), (-1.234, 1.234, 1), (f64::INFINITY, f64::INFINITY, 0), (f64::INFINITY, f64::NEG_INFINITY, 1), (f64::NEG_INFINITY, f64::NEG_INFINITY, 0), (f64::INFINITY, f64::NAN, 1), (f64::NAN, f64::NAN, 1)] {
    ...
    fn evaluate_binary_big() {
        ...
        assert_eq!(1, op.evaluate_binary_i128(1, 8, inf, ninf));
        assert_eq!(0, op.evaluate_binary_i128(1, 8, ninf, ninf));
        assert_eq!(1, op.evaluate_binary_i128(1, 8, inf, nan));
        assert_eq!(0, op.evaluate_binary_i128(1, 8, zero, zero));
    }
}
