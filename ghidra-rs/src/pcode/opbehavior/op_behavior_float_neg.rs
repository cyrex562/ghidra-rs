//! Port of `ghidra.pcode.opbehavior.OpBehaviorFloatNeg`.
...
/// FLOAT_NEG p-code operation behavior: floating-point negation.
///
...
///
/// Corresponds to `ghidra.pcode.opbehavior.OpBehaviorFloatNeg`.
...
impl OpBehaviorFloatNeg {
    /// Construct a new `OpBehaviorFloatNeg` for [`OpCode::FloatNeg`].
    pub fn new() -> Self {
        Self { base: OpBehavior::new(OpCode::FloatNeg as i32) }
    }
}

impl Default for OpBehaviorFloatNeg {
    fn default() -> Self {
        Self::new()
    }
}

impl UnaryOpBehavior for OpBehaviorFloatNeg {
    fn opcode(&self) -> i32 {
        self.base.opcode()
    }
    ...
    fn evaluate_unary_i128(&self, _sizeout: i32, sizein: i32, in1: i128) -> i128 {
        let format = format_for(sizein);
        big_int_to_i128(&format.op_neg_big(&BigInt::from(in1)))
    }
}
...
mod tests {
    ...
    fn opcode_matches_pcode_op() {
        assert_eq!(OpBehaviorFloatNeg::new().opcode(), OpCode::FloatNeg as i32);
    }

    #[test]
    fn evaluate_unary_long() {
        let op = OpBehaviorFloatNeg::new();
        let ff = ff(8);
        for (v, expect) in [(2.5, -2.5), (-2.5, 2.5), (f64::INFINITY, f64::NEG_INFINITY), (f64::NEG_INFINITY, f64::INFINITY), (f64::NAN, f64::NAN)] {
            assert_host(ff, expect, op.evaluate_unary_i64(8, 8, ff.get_encoding(v)));
        }
        // +0.0f -> -0.0f
        assert_eq!(0x80000000, op.evaluate_unary_i64(4, 4, 0));
    }
    ...
    fn evaluate_unary_big() {
        ...
        assert_eq!(ff.get_big_infinity(true), big_dec(ff, op.evaluate_unary_i128(8, 8, inf)));
        ...
        // binary128 1.0 -> -1.0 sets bit 127 (negative as i128)
        let one: i128 = 0x3fff << 112;
        assert_eq!(one | i128::MIN, op.evaluate_unary_i128(16, 16, one));
    }
}
