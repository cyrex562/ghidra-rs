//! Port of `ghidra.pcode.opbehavior.OpBehaviorFloatLessEqual`.
...
/// FLOAT_LESSEQUAL p-code operation behavior: floating-point less-than-or-equal (a one-byte boolean).
///
...
///
/// Corresponds to `ghidra.pcode.opbehavior.OpBehaviorFloatLessEqual`.
...
impl OpBehaviorFloatLessEqual {
    /// Construct a new `OpBehaviorFloatLessEqual` for [`OpCode::FloatLessEqual`].
    pub fn new() -> Self {
        Self { base: OpBehavior::new(OpCode::FloatLessEqual as i32) }
    }
}

impl Default for OpBehaviorFloatLessEqual {
    fn default() -> Self {
        Self::new()
    }
}

impl BinaryOpBehavior for OpBehaviorFloatLessEqual {
    fn opcode(&self) -> i32 {
        self.base.opcode()
    }
    ...
    fn evaluate_binary_i128(&self, _sizeout: i32, sizein: i32, in1: i128, in2: i128) -> i128 {
        let format = format_for(sizein);
        big_int_to_i128(&format.op_less_equal_big(&BigInt::from(in1), &BigInt::from(in2)))
    }
}
...
mod tests {
    ...
    fn opcode_matches_pcode_op() {
        assert_eq!(OpBehaviorFloatLessEqual::new().opcode(), OpCode::FloatLessEqual as i32);
    }

    #[test]
    fn evaluate_binary_long() {
        let op = OpBehaviorFloatLessEqual::new();
        let ff = ff(8);
        let e = |d: f64| ff.get_encoding(d);
        for (a, b, expect) in [(1.234, 1.234, 1), (-1.234, -1.234, 1), (1.234, -1.234, 0), (0.0, -1.234, 0), (0.0, 1.234, 1), (-1.234, 1.234, 1), (f64::INFINITY, 1.234, 0), (f64::NEG_INFINITY, 1.234, 1), (1.234, f64::INFINITY, 1), (1.234, f64::NEG_INFINITY, 0), (f64::INFINITY, f64::INFINITY, 1), (f64::NEG_INFINITY, f64::INFINITY, 1), (f64::NAN, f64::NAN, 0)] {
    ...
    fn evaluate_binary_big() {
        ...
        assert_eq!(1, op.evaluate_binary_i128(1, 8, nan, nan));
    }
}
