//! Port of `ghidra.pcode.opbehavior.OpBehaviorFloatCeil`.
...
/// FLOAT_CEIL p-code operation behavior: floating-point ceiling.
///
...
///
/// Corresponds to `ghidra.pcode.opbehavior.OpBehaviorFloatCeil`.
...
impl OpBehaviorFloatCeil {
    /// Construct a new `OpBehaviorFloatCeil` for [`OpCode::FloatCeil`].
    pub fn new() -> Self {
        Self { base: OpBehavior::new(OpCode::FloatCeil as i32) }
    }
}

impl Default for OpBehaviorFloatCeil {
    fn default() -> Self {
        Self::new()
    }
}

impl UnaryOpBehavior for OpBehaviorFloatCeil {
    fn opcode(&self) -> i32 {
        self.base.opcode()
    }
    ...
    fn evaluate_unary_i128(&self, _sizeout: i32, sizein: i32, in1: i128) -> i128 {
        let format = format_for(sizein);
        big_int_to_i128(&format.op_ceil_big(&BigInt::from(in1)))
    }
}
...
mod tests {
    ...
    fn opcode_matches_pcode_op() {
        assert_eq!(OpBehaviorFloatCeil::new().opcode(), OpCode::FloatCeil as i32);
    }

    #[test]
    fn evaluate_unary_long() {
        let op = OpBehaviorFloatCeil::new();
        let ff = ff(8);
        for (v, expect) in [(2.5, 3.0), (-2.5, -2.0), (f64::INFINITY, f64::INFINITY), (f64::NEG_INFINITY, f64::NEG_INFINITY), (f64::NAN, f64::NAN)] {
            assert_host(ff, expect, op.evaluate_unary_i64(8, 8, ff.get_encoding(v)));
        }
        // -0.5f ceils to -0.0f
        assert_eq!(0x80000000, op.evaluate_unary_i64(4, 4, 0xbf000000));
    }
